...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("v32_0_1_mechanism_probe_output.txt"),
        help="Text report path.",
    )
    parser.add_argument(
        "--csv",
        type=Path,
        default=Path("v32_0_1_mechanism_probe.csv"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Retained for CLI compatibility; v32.0.1 evaluates all exact ±E pairs.",
    )
    parser.add_argument(
        "--eta",
        type=float,
    ...
    # v32.0.1 is intentionally frozen to one seed: the Phase-3.1 reference
    # seed 25042000.  This isolates "global versus hotspot-specific" before
    # any across-seed generalization.
    if args.all_seeds:
        raise ValueError(
            "v32.0.1 is a one-seed control. Do not use --all-seeds."
        )
    if args.seed is not None and args.seed != HAMILTONIAN_SEEDS[0]:
        raise ValueError(
            f"v32.0.1 is frozen to seed {HAMILTONIAN_SEEDS[0]}."
        )
    seeds = (HAMILTONIAN_SEEDS[0],)
    ...
    report_lines.extend(pair_level_summary(all_rows))
