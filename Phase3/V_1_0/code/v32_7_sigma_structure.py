...
def main() -> None:
    ...
    parser.add_argument("--output",type=Path,default=Path("v32_7_sigma_structure_output.txt"))
    parser.add_argument("--csv",type=Path,default=Path("v32_7_sigma_structure.csv"))
    parser.add_argument("--fold-csv",type=Path,default=Path("v32_7_sigma_structure_folds.csv"))
    ...
    for i,seed in enumerate(HAMILTONIAN_SEEDS,start=1):
        ...
        for fam in FAMILIES:
            rows.extend(pair_metrics_for_family_v327(seed,fam,args.eta))

    report,folds=summary_v327(rows,args.alpha)
    print(report,end="")
    args.output.write_text(report,encoding="utf-8")
    write_rows_v327(args.csv,rows)
