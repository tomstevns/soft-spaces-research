...
def main() -> None:
    ...
    parser.add_argument("--output",type=Path,default=Path("v32_8_termwise_interference_output.txt"))
    parser.add_argument("--csv",type=Path,default=Path("v32_8_termwise_interference.csv"))
    parser.add_argument("--fold-csv",type=Path,default=Path("v32_8_termwise_interference_folds.csv"))
    args=parser.parse_args()

    if args.eta<=0: raise ValueError("--eta must be > 0")
    if args.alpha<0: raise ValueError("--alpha must be >= 0")

    rows=[]
    for i,seed in enumerate(HAMILTONIAN_SEEDS,start=1):
        print(f"[{i}/{len(HAMILTONIAN_SEEDS)}] seed {seed}",flush=True)
        for fam in FAMILIES:
            rows.extend(pair_metrics_for_family_v328(seed,fam,args.eta))

    report,folds=summary_v328(rows,args.alpha)
    print(report,end="")
    args.output.write_text(report,encoding="utf-8")
    write_rows_v328(args.csv,rows)
    write_fold_csv(args.fold_csv,folds)
