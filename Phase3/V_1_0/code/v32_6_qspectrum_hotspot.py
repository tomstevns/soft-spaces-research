...
def main() -> None:
    ...
    parser.add_argument("--output",type=Path,default=Path("v32_6_qspectrum_hotspot_output.txt"))
    parser.add_argument("--csv",type=Path,default=Path("v32_6_qspectrum_hotspot.csv"))
    parser.add_argument("--fold-csv",type=Path,default=Path("v32_6_qspectrum_hotspot_folds.csv"))
    ...
    for i,seed in enumerate(HAMILTONIAN_SEEDS,start=1):
        ...
        for fam in FAMILIES:
            rows.extend(pair_metrics_for_family_v326(seed,fam,args.eta))

    report,folds=summary_v326(rows,args.alpha)
    print(report,end="")
    args.output.write_text(report,encoding="utf-8")
    write_rows_v326(args.csv,rows)
