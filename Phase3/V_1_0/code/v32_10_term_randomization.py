...
def main() -> None:
    ...
    parser.add_argument("--nrand",type=int,default=N_RANDOMIZATIONS)
    parser.add_argument("--output",type=Path,default=Path("v32_10_term_randomization_output.txt"))
    parser.add_argument("--csv",type=Path,default=Path("v32_10_term_randomization.csv"))
    parser.add_argument("--stats-csv",type=Path,default=Path("v32_10_term_randomization_stats.csv"))
    args=parser.parse_args()

    if args.eta<=0: raise ValueError("--eta must be > 0")
    if args.nrand<10: raise ValueError("--nrand must be >= 10")

    rows=[]
    for i,seed in enumerate(HAMILTONIAN_SEEDS,start=1):
        print(f"[{i}/{len(HAMILTONIAN_SEEDS)}] seed {seed}",flush=True)
        for fam in FAMILIES:
            rows.extend(pair_metrics_for_family_v3210(seed,fam,args.eta,args.nrand))

    stats=seedwise_randomization_stats(rows)
    report=summary_v3210(rows,stats,args.nrand)
    print(report,end="")
    args.output.write_text(report,encoding="utf-8")
    write_rows_v3210(args.csv,rows)
    write_stats_v3210(args.stats_csv,stats)
