...
def fisher_z_mean(values: list[float]) -> float:
    vals=np.asarray([v for v in values if np.isfinite(v) and abs(v)<1.0],dtype=float)
...
def sign_consistency(values: list[float]) -> tuple[int,int,float]:
    vals=[v for v in values if np.isfinite(v) and abs(v)>1e-15]
    if not vals: return 0,0,float('nan')
    pos=sum(v>0 for v in vals); neg=sum(v<0 for v in vals)
    return pos,neg,max(pos,neg)/len(vals)

def candidate_keys_v324() -> list[str]:
    return [
        'real_sigma','real_sigma_over_gap',
        'delta_sigma','delta_sigma_over_gap',
        'local_real_sigma','local_delta_sigma',
        'real_pvq2','real_pvq2_over_gap','real_gap',
    ]

def seed_family_correlations(rows:list[dict]) -> list[dict]:
    out=[]
    for seed in sorted(set(r['seed'] for r in rows)):
        for fam in FAMILIES:
            sub=[r for r in rows if r['seed']==seed and r['family']==fam]
            if len(sub)<3: continue
            y=np.asarray([r['prominence'] for r in sub],dtype=float)
            for key in candidate_keys_v324():
                x=np.asarray([r[key] for r in sub],dtype=float)
                p,s=corr_pair(x,y)
                out.append({'seed':seed,'family':fam,'candidate':key,
                            'pearson':p,'spearman':s,'n_pairs':len(sub)})
    return out

def write_corr_csv(path:Path, rows:list[dict]) -> None:
    if not rows: return
    with path.open('w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader(); w.writerows(rows)

def across_seed_summary(rows:list[dict], recs:list[dict]) -> str:
    seeds=sorted(set(r['seed'] for r in rows))
    lines=[
        '=== Soft Spaces Phase 3.2 v32.4 ACROSS-SEED HOTSPOT MECHANISM CONFIRMATION ===',
        f'Seeds={len(seeds)} ({seeds[0]}..{seeds[-1]})',
        f'Total pair/family rows={len(rows)}',
        '',
        'Primary question:',
        '  Does the Sigma-based hotspot signal from v32.3 recur across the',
        '  frozen Hamiltonian seeds, and is transverse consistently stronger',
        '  than dephasing?',
        '',
        'Frozen response: Phase-3.1 prominence for each exact +/-E pair.',
        ''
    ]
    ...
    for fam in FAMILIES:
        ...
        ranked=[]
        for key in candidate_keys_v324():
            vals=[r['spearman'] for r in recs
                  if r['family']==fam and r['candidate']==key and np.isfinite(r['spearman'])]
            if not vals: continue
            med=float(np.median(vals)); mean=float(np.mean(vals)); fz=fisher_z_mean(vals)
            pos,neg,cons=sign_consistency(vals)
            ranked.append((abs(med),key,med,mean,fz,pos,neg,cons,min(vals),max(vals)))
        ranked.sort(reverse=True)
        for _,key,med,mean,fz,pos,neg,cons,vmin,vmax in ranked:
            lines.append(
                f'  {key:24s} median rho={med:+.4f}  mean={mean:+.4f}  '
                f'Fisher={fz:+.4f}  sign={max(pos,neg)}/{pos+neg} ({cons:.2f})  '
                f'range=[{vmin:+.4f},{vmax:+.4f}]'
            )
        lines.append('')

    lines.append('TRANSVERSE vs DEPHASING:')
    for key in ('real_sigma','real_sigma_over_gap'):
        diffs=[]
        for seed in seeds:
            vals={}
            for fam in FAMILIES:
                hit=[r for r in recs if r['seed']==seed and r['family']==fam
                     and r['candidate']==key and np.isfinite(r['spearman'])]
                if hit: vals[fam]=hit[0]['spearman']
            if all(f in vals for f in FAMILIES):
                diffs.append(vals['transverse']-vals['dephasing'])
        if diffs:
            a=np.asarray(diffs,dtype=float)
            lines.append(
                f'  {key:24s} median[rho_T-rho_D]={np.median(a):+.4f}; '
                f'T>D in {np.sum(a>0)}/{len(a)} seeds; '
                f'range=[{np.min(a):+.4f},{np.max(a):+.4f}]'
            )
    lines.append('')

    lines.append('POOLED TERTILE DIRECTION CHECK:')
    for fam in FAMILIES:
        for key in ('real_sigma','real_sigma_over_gap'):
            trip=[]
            for seed in seeds:
                sub=sorted([r for r in rows if r['seed']==seed and r['family']==fam],
                           key=lambda r:r['prominence'], reverse=True)
                if len(sub)<6: continue
                n=len(sub); c1=(n+2)//3; c2=(2*n+2)//3
                meds=[float(np.median([r[key] for r in sub[:c1]])),
                      float(np.median([r[key] for r in sub[c1:c2]])),
                      float(np.median([r[key] for r in sub[c2:]]))]
                scale=max(float(np.median([abs(r[key]) for r in sub])),1e-15)
                trip.append([m/scale for m in meds])
            if trip:
                a=np.asarray(trip,dtype=float); m=np.median(a,axis=0)
                monotonic=int(np.sum((a[:,0]>a[:,1])&(a[:,1]>a[:,2])))
                lines.append(
                    f'  {fam:10s} {key:20s} normalized TOP/MID/BOT='
                    f'{m[0]:.3f}/{m[1]:.3f}/{m[2]:.3f}; '
                    f'TOP>MID>BOTTOM in {monotonic}/{len(a)} seeds'
                )
    lines.append('')
    ...
    def stats(fam,key):
        ...
        _,_,cons=sign_consistency(vals)
        return float(np.median(vals)),cons

    ts,tsc=stats('transverse','real_sigma')
    tg,tgc=stats('transverse','real_sigma_over_gap')
    ds,dsc=stats('dephasing','real_sigma')
    dg,dgc=stats('dephasing','real_sigma_over_gap')

    lines += [
        '=== DECISION GUIDE ===',
        f'  transverse real_sigma:          median rho={ts:+.4f}, sign consistency={tsc:.2f}',
        f'  transverse real_sigma_over_gap: median rho={tg:+.4f}, sign consistency={tgc:.2f}',
        f'  dephasing  real_sigma:          median rho={ds:+.4f}, sign consistency={dsc:.2f}',
        f'  dephasing  real_sigma_over_gap: median rho={dg:+.4f}, sign consistency={dgc:.2f}',
        ''
    ]

    if max(abs(ts),abs(tg))>=0.5 and max(tsc,tgc)>=0.75:
        lines += [
            'RESULT: TRANSVERSE SIGMA SIGNAL CONFIRMED ACROSS SEEDS.',
            'The v32.3 association is not confined to the reference seed.'
        ]
    else:
        lines += [
            'RESULT: TRANSVERSE SIGMA SIGNAL NOT YET STABLE ACROSS SEEDS.',
            'The v32.3 association does not generalize cleanly enough.'
        ]

    if max(abs(ds),abs(dg)) < max(abs(ts),abs(tg)):
        lines.append('Dephasing remains weaker/different, supporting family dependence.')
    else:
        lines.append('Dephasing is not clearly weaker; family dependence must be re-evaluated.')

    lines += [
        '',
        'This is a confirmation/discrimination test, not a universal hotspot law.'
    ]
    return '\n'.join(lines)+'\n'
...
def main() -> None:
    ...
    parser.add_argument('--output',type=Path,default=Path('v32_4_hotspot_mechanism_confirmation_output.txt'))
    parser.add_argument('--csv',type=Path,default=Path('v32_4_hotspot_mechanism_confirmation.csv'))
    parser.add_argument('--corr-csv',type=Path,default=Path('v32_4_hotspot_mechanism_correlations.csv'))
    parser.add_argument('--seeds',type=int,default=len(HAMILTONIAN_SEEDS),
                        help=f'Number of frozen seeds from start of list (1..{len(HAMILTONIAN_SEEDS)}).')
    args=parser.parse_args()

    if args.eta<=0: raise ValueError('--eta must be > 0')
    if args.seeds<1 or args.seeds>len(HAMILTONIAN_SEEDS):
        raise ValueError(f'--seeds must be between 1 and {len(HAMILTONIAN_SEEDS)}')

    selected=HAMILTONIAN_SEEDS[:args.seeds]
    rows=[]
    for i,seed in enumerate(selected,start=1):
        print(f'[{i}/{len(selected)}] seed {seed}',flush=True)
        for family in FAMILIES:
            rows.extend(pair_metrics_for_family(seed,family,args.eta))

    recs=seed_family_correlations(rows)
    report=across_seed_summary(rows,recs)
    print(report,end='')
    args.output.write_text(report,encoding='utf-8')
    write_local_csv(args.csv,rows)
    write_corr_csv(args.corr_csv,recs)
