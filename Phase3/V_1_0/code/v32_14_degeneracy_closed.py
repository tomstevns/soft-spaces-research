...
# ---------------------------------------------------------------------------
# v32.10 direct mechanism test: destroy term alignment, preserve term norms
# ---------------------------------------------------------------------------
...
def coherence_from_terms(terms:list[np.ndarray]) -> float:
    if not terms:
        return 0.0
    total = np.sum(terms, axis=0)
    denom = sum(float(np.linalg.norm(t, ord="fro")) for t in terms)
    if denom <= 0:
        return 0.0
    return float(np.linalg.norm(total, ord="fro") / denom)


def randomized_coherence_signflip(
    terms:list[np.ndarray],
    rng:np.random.Generator,
    n_randomizations:int,
) -> tuple[float,float,float]:
    """
    Random independent ±1 signs on T_q.
    Each individual ||T_q||_F is exactly preserved.
    Returns mean, std, and fraction of randomized C >= observed C.
    """
    observed = coherence_from_terms(terms)
    vals = np.empty(n_randomizations, dtype=float)
    arr = np.asarray(terms)
    denom = sum(float(np.linalg.norm(t, ord="fro")) for t in terms)
    for k in range(n_randomizations):
        signs = rng.choice(np.array([-1.0, 1.0]), size=len(terms))
        total = np.tensordot(signs, arr, axes=(0,0))
        vals[k] = float(np.linalg.norm(total, ord="fro") / denom) if denom > 0 else 0.0
    p_ge = float((1 + np.sum(vals >= observed)) / (n_randomizations + 1))
    return float(np.mean(vals)), float(np.std(vals)), p_ge


def pair_metrics_for_family_v3210(seed:int, family:str, eta:float, nrand:int) -> list[dict]:
    rows = pair_metrics_for_family(seed, family, eta)
    ...
    for gi, group in enumerate(groups):
        ...
        c_obs = coherence_from_terms(terms)
        rng = np.random.default_rng(stable_hash_int(
            f"{VERSION}|{family}|{seed}|group={gi}|signflip"
        ))
        c_null_mean, c_null_std, p_ge = randomized_coherence_signflip(
            terms, rng, nrand
        )
        z = (c_obs - c_null_mean) / c_null_std if c_null_std > 1e-15 else 0.0
        desc[gi] = {
            "coherence_obs": c_obs,
            "coherence_rand_mean": c_null_mean,
            "coherence_rand_std": c_null_std,
            "coherence_excess": c_obs - c_null_mean,
            "coherence_z": z,
            "coherence_rand_p_ge": p_ge,
        }

    for row in rows:
        gm = int(row["g_minus"])
        gp = int(row["g_plus"])
        dm = desc[gm]; dp = desc[gp]
        for key in dm:
            row[key] = 0.5 * (float(dm[key]) + float(dp[key]))
    return rows


def seedwise_randomization_stats(rows:list[dict]) -> list[dict]:
    out=[]
    keys=[
        "coherence_obs",
        "coherence_excess",
    ...
    for seed in sorted(set(r["seed"] for r in rows)):
        for fam in FAMILIES:
            ...
            for key in keys:
                ...
                p,s=corr_pair(x,y)
                out.append({
                    "seed":seed,"family":fam,"metric":key,
                    "pearson":p,"spearman":s,"n_pairs":len(sub),
                })
    return out


def write_stats_v3210(path:Path, rows:list[dict]) -> None:
    if not rows: return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader(); w.writerows(rows)
...
def summary_v3210(rows:list[dict], stats:list[dict], nrand:int) -> str:
    lines=[
        "=== Soft Spaces Phase 3.2 v32.10 CAUSAL TERM-RANDOMIZATION TEST ===",
        f"Seeds={len(set(r['seed'] for r in rows))}",
        f"Random sign-flips per eigenspace={nrand}",
        "",
        "Mechanistic intervention:",
        "  Randomize independent signs of T_q contributions.",
        "  Each individual ||T_q||_F is preserved exactly.",
        "  Only constructive/destructive alignment between contributions is destroyed.",
        "",
        "Primary test:",
        "  Does observed coherence exceed randomized coherence, and does the",
        "  excess above that norm-preserving null track hotspot prominence?",
        "",
    ]
    ...
    for fam in FAMILIES:
        ...
        for key in ["coherence_obs","coherence_excess","coherence_z","coherence_rand_mean"]:
            vals=[r["spearman"] for r in stats
                  if r["family"]==fam and r["metric"]==key and np.isfinite(r["spearman"])]
            pos=sum(v>0 for v in vals)
            lines.append(
                f"  {key:24s} median seed rho={np.median(vals):+.4f}; "
                f"mean={np.mean(vals):+.4f}; positive={pos}/{len(vals)}; "
                f"range=[{np.min(vals):+.4f},{np.max(vals):+.4f}]"
            )

        y=np.asarray([r["prominence"] for r in famrows],float)
        for key in ["coherence_obs","coherence_excess","coherence_z","coherence_rand_mean"]:
            x=np.asarray([r[key] for r in famrows],float)
            p,s=corr_pair(x,y)
            lines.append(
                f"  pooled {key:17s} Pearson={p:+.4f}; Spearman={s:+.4f}"
            )

        obs=np.asarray([r["coherence_obs"] for r in famrows],float)
        nul=np.asarray([r["coherence_rand_mean"] for r in famrows],float)
        excess=obs-nul
        pvals=np.asarray([r["coherence_rand_p_ge"] for r in famrows],float)

        lines += [
            f"  median observed C_E       = {np.median(obs):.6f}",
            f"  median randomized C_E     = {np.median(nul):.6f}",
            f"  median coherence excess   = {np.median(excess):+.6f}",
            f"  fraction pair rows C_obs>C_null_mean = {np.mean(obs>nul):.3f}",
            f"  fraction pair rows p<=0.05 = {np.mean(pvals<=0.05):.3f}",
            "",
        ]

    lines += [
        "=== DECISION GUIDE ===",
        "Support for a mechanistic coherence role requires BOTH:",
        "  A) observed C_E systematically differs from the norm-preserving randomized null,",
        "  B) coherence excess/z retains a positive cross-seed association with prominence.",
        "",
        "If A and B hold, the hotspot signal depends on alignment/cancellation",
        "between virtual Q pathways, not merely on their individual magnitudes.",
        "This is stronger than correlation, but still a computational intervention",
        "rather than a complete analytic derivation.",
    ]
    return "\n".join(lines)+"\n"


def write_rows_v3210(path:Path, rows:list[dict]) -> None:
    if not rows: return
    keys=list(rows[0].keys())
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=keys)
...
# ---------------------------------------------------------------------------
# v32.11 strict confirmation: vectorized norm-preserving sign null
# ---------------------------------------------------------------------------

N_RANDOMIZATIONS_V3211 = 1000
N_SEED_PERMUTATIONS = 20000

def randomized_coherence_signflip_fast(
    terms:list[np.ndarray],
    rng:np.random.Generator,
    n_randomizations:int,
    batch:int=250,
    ...
    observed = coherence_from_terms(terms)
    arr = np.asarray(terms, dtype=np.complex128)
    denom = sum(float(np.linalg.norm(t, ord="fro")) for t in terms)
    if denom <= 0:
        return 0.0, 0.0, 1.0
    ...
    while done<n_randomizations:
        b=min(batch,n_randomizations-done)
        signs=rng.choice(np.array([-1.0,1.0]), size=(b,len(terms)))
        totals=np.tensordot(signs,arr,axes=(1,0))  # b x d x d
        norms=np.sqrt(np.sum(np.abs(totals)**2,axis=(1,2))) / denom
        vals.append(norms)
        done += b
...
def pair_metrics_for_family_v3211(seed:int, family:str, eta:float, nrand:int) -> list[dict]:
    rows = pair_metrics_for_family(seed, family, eta)
    ...
    # Only evaluate one member of each exact ±E pair; v32.2 established
    # pair covariance for the REAL model, so this avoids redundant work.
    desc={}
    needed=sorted(set(int(r["g_minus"]) for r in rows))
    for gi in needed:
        terms=term_matrices(
            eigenvalues=evals,
            eigenvectors=evecs,
        ...
        obs=coherence_from_terms(terms)
        rng=np.random.default_rng(stable_hash_int(
            f"{VERSION}|{family}|{seed}|group={gi}|fast-signflip"
        ))
        null_mean,null_std,p_ge=randomized_coherence_signflip_fast(
            terms,rng,nrand
        )
        z=(obs-null_mean)/null_std if null_std>1e-15 else 0.0
        desc[gi]={
            "coherence_obs":obs,
            "coherence_rand_mean":null_mean,
            "coherence_rand_std":null_std,
            "coherence_excess":obs-null_mean,
            "coherence_z":z,
            "coherence_rand_p_ge":p_ge,
        }

    for row in rows:
        d=desc[int(row["g_minus"])]
        row.update({k:float(v) for k,v in d.items()})
    return rows


def permutation_p_rho(x:np.ndarray, y:np.ndarray, seed:int, nperm:int) -> tuple[float,float]:
    obs=corr_pair(x,y)[1]
    rng=np.random.default_rng(seed)
    count=0
    for _ in range(nperm):
        ...
        rp=corr_pair(x,yp)[1]
        if np.isfinite(rp) and rp>=obs:
            count+=1
    return obs, float((count+1)/(nperm+1))


def binomial_sign_p(k:int, n:int) -> float:
    # one-sided P[X >= k], X~Binomial(n,0.5)
    return sum(math.comb(n,j) for j in range(k,n+1)) / (2**n)


def strict_seed_stats(rows:list[dict], nperm:int) -> list[dict]:
    out=[]
    for seed in sorted(set(r["seed"] for r in rows)):
        for fam in FAMILIES:
            sub=[r for r in rows if r["seed"]==seed and r["family"]==fam]
            y=np.asarray([r["prominence"] for r in sub],float)
            for metric in ["coherence_obs","coherence_excess","coherence_z"]:
                x=np.asarray([r[metric] for r in sub],float)
                rho,p=permutation_p_rho(
                    x,y,stable_hash_int(f"{VERSION}|perm|{seed}|{fam}|{metric}"),nperm
                )
                out.append({
                    "seed":seed,"family":fam,"metric":metric,
                    "spearman":rho,"perm_p_one_sided":p,"n_pairs":len(sub),
                })
    return out


def write_strict_stats(path:Path, rows:list[dict]) -> None:
    if not rows:return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader();w.writerows(rows)
...
def summary_v3211(rows:list[dict], stats:list[dict], nrand:int, nperm:int) -> str:
    lines=[
        "=== Soft Spaces Phase 3.2 v32.11 STRICT COHERENCE-EXCESS CONFIRMATION ===",
        f"Seeds={len(set(r['seed'] for r in rows))}",
        f"Norm-preserving sign randomizations per pair={nrand}",
        f"Seed-level Spearman permutations={nperm}",
        "",
        "Primary endpoint:",
        "  coherence_excess = C_observed - mean(C_sign-randomized)",
        "",
        "Confirmation criteria:",
        "  C1: median seedwise rho > 0",
        "  C2: positive rho in >= 9/12 seeds",
        "  C3: one-sided sign-test p <= 0.05",
        "  C4: pooled Spearman rho > 0",
        "  C5: family-level median coherence excess > 0",
        "",
    ]
    ...
    for fam in FAMILIES:
        ...
        for metric in ["coherence_obs","coherence_excess","coherence_z"]:
            ss=[r for r in stats if r["family"]==fam and r["metric"]==metric]
            vals=np.asarray([r["spearman"] for r in ss],float)
            pos=int(np.sum(vals>0))
            sig=int(np.sum(np.asarray([r["perm_p_one_sided"] for r in ss])<=0.05))
            x=np.asarray([r[metric] for r in famrows],float)
            pooled=corr_pair(x,y)[1]
            lines.append(
                f"  {metric:20s} median rho={np.median(vals):+.4f}; "
                f"positive={pos}/{len(vals)}; sign-p={binomial_sign_p(pos,len(vals)):.5f}; "
                f"seed perm p<=0.05: {sig}/{len(vals)}; pooled rho={pooled:+.4f}"
            )

        ex=np.asarray([r["coherence_excess"] for r in famrows],float)
        pnull=np.asarray([r["coherence_rand_p_ge"] for r in famrows],float)
        lines += [
            f"  median coherence excess={np.median(ex):+.6f}",
            f"  pair fraction excess>0={np.mean(ex>0):.3f}",
            f"  pair fraction randomization p<=0.05={np.mean(pnull<=0.05):.3f}",
        ]

        ss=[r for r in stats if r["family"]==fam and r["metric"]=="coherence_excess"]
        vals=np.asarray([r["spearman"] for r in ss],float)
        pos=int(np.sum(vals>0))
        pooled=corr_pair(
            np.asarray([r["coherence_excess"] for r in famrows],float),y
        )[1]
        c1=np.median(vals)>0
        c2=pos>=9
        c3=binomial_sign_p(pos,len(vals))<=0.05
        c4=pooled>0
        c5=np.median(ex)>0
        passed=sum([c1,c2,c3,c4,c5])
        lines.append(
            f"  GATE: C1={c1} C2={c2} C3={c3} C4={c4} C5={c5} => {passed}/5"
        )
        if passed==5:
            lines.append("  RESULT: STRONG CONFIRMATION")
        elif passed>=4:
            lines.append("  RESULT: MODERATE CONFIRMATION")
        else:
            lines.append("  RESULT: NOT CONFIRMED")
        lines.append("")

    lines += [
        "=== INTERPRETATION ===",
        "Passing all five gates supports a cross-seed mechanistic role for",
        "coherent/cancelling alignment among the virtual Q pathways.",
        "It still does not by itself constitute an analytic derivation of prominence.",
    ]
    return "\n".join(lines)+"\n"


def write_rows_v3211(path:Path, rows:list[dict]) -> None:
    if not rows:return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader();w.writerows(rows)
...
# ---------------------------------------------------------------------------
# v32.12 provisional local hotspot law
# ---------------------------------------------------------------------------
...
#   C_E = || sum_q T_q ||_F / sum_q ||T_q||_F
#
# and the null independently randomizes the signs of T_q while preserving
# every individual ||T_q||_F.
#
# The tested hypothesis is not equality to prominence, but monotone ordering:
#
#   prominence(E) increases with H_E.
#
# This is the narrowest law directly supported by v32.11.

N_RANDOMIZATIONS_V3212 = 2000
N_PERM_V3212 = 5000

def pair_metrics_for_family_v3212(seed:int, family:str, eta:float, nrand:int) -> list[dict]:
    rows = pair_metrics_for_family(seed, family, eta)
    ...
    # Evaluate only one member of each symmetry-related ±E pair.
    desc={}
    for gi in sorted(set(int(r["g_minus"]) for r in rows)):
        terms=term_matrices(
            eigenvalues=evals,
            eigenvectors=evecs,
            group=groups[gi],
        ...
        c_obs=coherence_from_terms(terms)
        rng=np.random.default_rng(stable_hash_int(
            f"{VERSION}|{family}|{seed}|g={gi}|law-null"
        ))
        c_null_mean,c_null_std,p_ge=randomized_coherence_signflip_fast(
            terms,rng,nrand
        )
        h_local=c_obs-c_null_mean
        h_z=h_local/c_null_std if c_null_std>1e-15 else 0.0
        desc[gi]={
            "C_E":c_obs,
            "C_null":c_null_mean,
            "H_local":h_local,
    ...
    for r in rows:
        r.update({k:float(v) for k,v in desc[int(r["g_minus"])].items()})
    return rows


def seed_law_stats(rows:list[dict], nperm:int) -> list[dict]:
    out=[]
    for seed in sorted(set(r["seed"] for r in rows)):
        for fam in FAMILIES:
            sub=[r for r in rows if r["seed"]==seed and r["family"]==fam]
            y=np.asarray([r["prominence"] for r in sub],float)
            for metric in ["H_local","H_z","C_E"]:
                x=np.asarray([r[metric] for r in sub],float)
                rho,p=permutation_p_rho(
                    x,y,stable_hash_int(f"{VERSION}|{seed}|{fam}|{metric}|perm"),nperm
                )
                out.append({
                    "seed":seed,"family":fam,"metric":metric,
                    "spearman":rho,"perm_p_one_sided":p,"n_pairs":len(sub)
...
def summary_v3212(rows:list[dict], stats:list[dict], nrand:int, nperm:int) -> str:
    lines=[
        "=== Soft Spaces Phase 3.2 v32.12 PROVISIONAL LOCAL HOTSPOT LAW TEST ===",
        f"Seeds={len(set(r['seed'] for r in rows))}",
        f"Null randomizations per pair={nrand}",
        f"Seed-level permutations={nperm}",
        "",
        ...
        "",
        "Physical reading:",
        "  H_E measures the excess constructive operator-coherence of the actual",
        "  virtual Q pathways above a norm-preserving sign-randomized reference.",
        "",
        "Hypothesis:",
        "  hotspot prominence is a monotone increasing function of H_E.",
        "",
    ]
    ...
    for fam in FAMILIES:
        ...
        for metric in ["H_local","H_z","C_E"]:
            ...
            sig=int(np.sum(np.asarray([r["perm_p_one_sided"] for r in ss])<=0.05))
            pooled=corr_pair(
                np.asarray([r[metric] for r in famrows],float),y
            )[1]
            lines.append(
                f"  {metric:8s} median seed rho={np.median(vals):+.4f}; "
                f"positive={pos}/{len(vals)}; sign-p={binomial_sign_p(pos,len(vals)):.5f}; "
                f"seed p<=0.05={sig}/{len(vals)}; pooled rho={pooled:+.4f}"
            )

        h=np.asarray([r["H_local"] for r in famrows],float)
        prom=np.asarray([r["prominence"] for r in famrows],float)

        # Within-seed prominence tertiles, normalized H by within-seed median |H|.
        trip=[]
        strict=0
        for seed in sorted(set(r["seed"] for r in famrows)):
            sr=sorted(
                [r for r in famrows if r["seed"]==seed],
                key=lambda r:r["prominence"], reverse=True
            )
            n=len(sr); c1=(n+2)//3; c2=(2*n+2)//3
            vals=[
                float(np.median([r["H_local"] for r in sr[:c1]])),
                float(np.median([r["H_local"] for r in sr[c1:c2]])),
                float(np.median([r["H_local"] for r in sr[c2:]])),
            ]
            scale=max(float(np.median([abs(r["H_local"]) for r in sr])),1e-15)
            trip.append([v/scale for v in vals])
            strict += int(vals[0]>vals[1]>vals[2])
        arr=np.asarray(trip,float)
        med=np.median(arr,axis=0)

        lines += [
            f"  median H_local={np.median(h):+.6f}",
            f"  fraction H_local>0={np.mean(h>0):.3f}",
            f"  normalized H TOP/MID/BOT={med[0]:+.3f}/{med[1]:+.3f}/{med[2]:+.3f}",
            f"  strict TOP>MID>BOTTOM in {strict}/{len(arr)} seeds",
            "",
        ]

    lines += [
        "=== DECISION ===",
        "The provisional law is retained if H_local:",
        "  1) has positive median seedwise rho in both families,",
        "  2) is positive in at least 9/12 seedwise correlations in both families,",
        "  3) passes the one-sided sign test at p<=0.05 in both families,",
        "  4) has positive pooled rho in both families.",
        "",
        "Passing these gates supports H_E as a compact empirical local-law parameter.",
        "It does not yet prove a closed analytic formula for prominence amplitude.",
    ]
    return "\n".join(lines)+"\n"


def write_rows_v3212(path:Path, rows:list[dict]) -> None:
    if not rows:return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader();w.writerows(rows)
...
def write_stats_v3212(path:Path, rows:list[dict]) -> None:
    if not rows:return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader();w.writerows(rows)
...
# ---------------------------------------------------------------------------
# v32.13 physically motivated law forms
# ---------------------------------------------------------------------------

N_RANDOMIZATIONS_V3213 = 1000
N_PERM_V3213 = 2000

LAW_FORMS = [
    "H_local",
    "H_times_sigma",
    "H_times_pvq2_over_gap",
    "C_times_sigma",
]

def pair_metrics_for_family_v3213(seed:int, family:str, eta:float, nrand:int) -> list[dict]:
    rows = pair_metrics_for_family_v3212(seed, family, eta, nrand)

    for r in rows:
        gap = max(float(r["real_gap"]), 1e-15)
        h = float(r["H_local"])
        c = float(r["C_E"])
        sigma = float(r["real_sigma"])
        pvq2 = float(r["real_pvq2"])

        r["H_times_sigma"] = h * sigma
        r["H_times_pvq2_over_gap"] = h * (pvq2 / gap)
        r["C_times_sigma"] = c * sigma

    return rows


def seed_law_form_stats(rows:list[dict], nperm:int) -> list[dict]:
    out=[]
    for seed in sorted(set(r["seed"] for r in rows)):
        for fam in FAMILIES:
            sub=[r for r in rows if r["seed"]==seed and r["family"]==fam]
            y=np.asarray([r["prominence"] for r in sub],float)
            for metric in LAW_FORMS:
                x=np.asarray([r[metric] for r in sub],float)
                rho,p=permutation_p_rho(
                    x,y,stable_hash_int(f"{VERSION}|{seed}|{fam}|{metric}|perm"),nperm
                )
...
def loso_single_metric(rows:list[dict], family:str, metric:str):
    sub=[r for r in rows if r["family"]==family]
    seeds=np.asarray([int(r["seed"]) for r in sub],int)
    x=np.asarray([float(r[metric]) for r in sub],float)
    y=np.asarray([float(r["prominence"]) for r in sub],float)

    preds=np.empty_like(y)
    fold=[]
    for held in sorted(set(seeds)):
        tr=seeds!=held
        te=seeds==held

        mu=float(np.mean(x[tr])); sd=float(np.std(x[tr]))
        if sd<1e-15: sd=1.0
        ztr=(x[tr]-mu)/sd
        zte=(x[te]-mu)/sd

        X=np.column_stack([np.ones(np.sum(tr)), ztr])
        beta=np.linalg.lstsq(X,y[tr],rcond=None)[0]
        pred=beta[0]+beta[1]*zte
        preds[te]=pred
        ...
        fold.append({
            ...
            "metric":metric,
            "held_seed":int(held),
            "spearman":corr_pair(y[te],pred)[1],
            "r2":r2_score_np(y[te],pred),
            "rmse":rmse_np(y[te],pred),
        })

    return {
        "pooled_spearman":corr_pair(y,preds)[1],
        "pooled_r2":r2_score_np(y,preds),
        "pooled_rmse":rmse_np(y,preds),
        "folds":fold,
    }


def summary_v3213(rows:list[dict], stats:list[dict]) -> str:
    lines=[
        "=== Soft Spaces Phase 3.2 v32.13 LOCAL LAW-FORM COMPARISON ===",
        f"Seeds={len(set(r['seed'] for r in rows))}",
        "",
        "Candidate law forms:",
        "  L1 = H_E",
        "  L2 = H_E * ||Sigma_E||_F",
        "  L3 = H_E * ||PVQ||_F^2 / Delta_PQ",
        "  L4 = C_E * ||Sigma_E||_F",
        "",
        "Goal:",
        "  Compare a small set of physically motivated local-law forms.",
        "  No broad feature search is performed.",
        "",
    ]
    ...
    for fam in FAMILIES:
        ...
        for metric in LAW_FORMS:
            ...
            loso=loso_single_metric(rows,fam,metric)

            ranked.append((
                loso["pooled_spearman"], metric, np.median(vals), pos, sig, pooled,
                loso["pooled_r2"], loso["pooled_rmse"]
            ))

        ranked.sort(reverse=True, key=lambda t:t[0])

        for rank,(lrho,metric,med,pos,sig,pooled,r2,rmse) in enumerate(ranked,1):
            lines.append(
                f"  #{rank} {metric:24s} "
                f"LOSO rho={lrho:+.4f}; R2={r2:+.4f}; "
                f"seed median rho={med:+.4f}; positive={pos}/12; "
                f"seed p<=0.05={sig}/12; pooled rho={pooled:+.4f}"
            )
        lines.append("")

    lines += [
        "=== DECISION ===",
        "Prefer a law form only if it improves over H_E alone in a reproducible",
        "cross-seed manner, especially LOSO Spearman and sign consistency.",
        "",
        "If no candidate clearly improves on H_E, retain H_E as the compact local",
        "mechanism parameter and move to analytic derivation rather than further fitting.",
    ]
    return "\n".join(lines)+"\n"


def write_stats_v3213(path:Path, rows:list[dict]) -> None:
    if not rows:return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader();w.writerows(rows)
...
# ---------------------------------------------------------------------------
# v32.14 degeneracy-closed, basis-invariant interference law
# ---------------------------------------------------------------------------

N_PERM_V3214 = 2000

def degeneracy_closed_terms(
    *,
    eigenvalues: np.ndarray,
    eigenvectors: np.ndarray,
    groups: list[np.ndarray],
    ...
    """
    Build basis-invariant Q-group contributions

        T_G = g_eta(E-E_G) P V P_G V P

    for every complete degeneracy-closed Q eigenspace G != target P.
    """
    p_group = groups[target_group_number]
    p_vec = eigenvectors[:, p_group]
    energy = eigenspace_energy(eigenvalues, p_group)

    acted_p = apply_pauli_sum(p_vec, perturbation)

    terms = []
    for gi, g in enumerate(groups):
        if gi == target_group_number:
            continue

        g_vec = eigenvectors[:, g]
        Eg = eigenspace_energy(eigenvalues, g)
        delta = energy - Eg
        weight = regularized_inverse_scalar(np.asarray([delta]), eta)[0]

        # P_G V P in basis form; invariant under unitary rotations inside G
        gv_p = g_vec.conj().T @ acted_p
        tg = weight * (gv_p.conj().T @ gv_p)
        tg = 0.5 * (tg + tg.conj().T)
        terms.append(tg)
...
def interference_descriptors_group_closed(terms:list[np.ndarray]) -> dict:
    if not terms:
        return {
            "C_group":0.0,
            "A_group":0.0,
    ...
    sigma = np.sum(terms, axis=0)
    sigma2 = float(np.linalg.norm(sigma, ord="fro")**2)
    self_sum = float(sum(np.linalg.norm(t, ord="fro")**2 for t in terms))
    term_l1 = float(sum(np.linalg.norm(t, ord="fro") for t in terms))

    cross_sum = sigma2 - self_sum
    denom = term_l1**2 if term_l1 > 0 else 1.0

    C_group = float(np.sqrt(max(sigma2,0.0)) / term_l1) if term_l1 > 0 else 0.0
    A_group = float(cross_sum / denom) if denom > 0 else 0.0
...
def pair_metrics_for_family_v3214(seed:int, family:str, eta:float) -> list[dict]:
    rows = pair_metrics_for_family(seed, family, eta)
    ...
    for gi in needed:
        ...
        desc[gi] = interference_descriptors_group_closed(terms)

    for r in rows:
        d = desc[int(r["g_minus"])]
        for k,v in d.items():
            r[k]=float(v)
    return rows


def seed_stats_v3214(rows:list[dict], nperm:int) -> list[dict]:
    out=[]
    for seed in sorted(set(r["seed"] for r in rows)):
        for fam in FAMILIES:
            sub=[r for r in rows if r["seed"]==seed and r["family"]==fam]
            ...
            for metric in ["A_group","C_group","cross_sum"]:
                ...
                rho,p=permutation_p_rho(
                    x,y,
                    stable_hash_int(f"{VERSION}|perm|{seed}|{fam}|{metric}"),
                    nperm
                )
                out.append({
                    "seed":seed,
...
def summary_v3214(rows:list[dict], stats:list[dict]) -> str:
    lines=[
        "=== Soft Spaces Phase 3.2 v32.14 DEGENERACY-CLOSED INTERFERENCE LAW ===",
        f"Seeds={len(set(r['seed'] for r in rows))}",
        "",
        "Basis-invariant Q-group decomposition:",
        "  T_G = g_eta(E-E_G) P V P_G V P",
        "  Sigma_E = sum_G T_G",
        "",
        "Primary analytic interference parameter:",
        "  A_E^(G) = [||Sigma_E||_F^2 - sum_G ||T_G||_F^2] / (sum_G ||T_G||_F)^2",
        "",
        "Equivalent cross-term form:",
        "  A_E^(G) = 2 sum_{G<K} Re Tr(T_G^dagger T_K) / (sum_G ||T_G||_F)^2",
        "",
        "Question:",
        "  Does the hotspot association survive when all Q contributions are",
        "  defined by complete degeneracy-closed projectors?",
        "",
    ]
    ...
    for fam in FAMILIES:
        ...
        for metric in ["A_group","C_group","cross_sum"]:
            ...
            sig=int(np.sum(np.asarray([r["perm_p_one_sided"] for r in ss])<=0.05))
            pooled=corr_pair(
                np.asarray([r[metric] for r in famrows],float),
                y
            )[1]
            lines.append(
        ...
        for seed in sorted(set(r["seed"] for r in famrows)):
            ...
            n=len(sr); c1=(n+2)//3; c2=(2*n+2)//3
            vals=[
                float(np.median([r["A_group"] for r in sr[:c1]])),
                float(np.median([r["A_group"] for r in sr[c1:c2]])),
                float(np.median([r["A_group"] for r in sr[c2:]])),
            ]
            scale=max(float(np.median([abs(r["A_group"]) for r in sr])),1e-15)
            trip.append([v/scale for v in vals])
            strict += int(vals[0] > vals[1] > vals[2])
        arr=np.asarray(trip,float)
        med=np.median(arr,axis=0)
        lines.append(
            f"  A_group normalized TOP/MID/BOT={med[0]:+.3f}/{med[1]:+.3f}/{med[2]:+.3f}; "
            f"strict TOP>MID>BOTTOM in {strict}/{len(arr)} seeds"
        )
        lines.append("")
    ...
    lines += [
        "=== DECISION ===",
        "If A_group remains positively associated with prominence in both families,",
        "the interference mechanism survives the degeneracy-closure correction.",
        "That would make A_E^(G) the preferred basis-invariant local-law candidate.",
    ]
    return "\n".join(lines)+"\n"


def write_rows_v3214(path:Path, rows:list[dict]) -> None:
    if not rows:return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader();w.writerows(rows)
...
def write_stats_v3214(path:Path, rows:list[dict]) -> None:
    if not rows:return
    with path.open("w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0].keys()))
        w.writeheader();w.writerows(rows)
...
def main() -> None:
    ...
    parser.add_argument("--nperm",type=int,default=N_PERM_V3214)
    parser.add_argument("--seed-start",type=int,default=0)
    parser.add_argument("--seed-count",type=int,default=len(HAMILTONIAN_SEEDS))
    parser.add_argument("--output",type=Path,default=Path("v32_14_degeneracy_closed_output.txt"))
    parser.add_argument("--csv",type=Path,default=Path("v32_14_degeneracy_closed.csv"))
    parser.add_argument("--stats-csv",type=Path,default=Path("v32_14_degeneracy_closed_stats.csv"))
    args=parser.parse_args()

    selected=HAMILTONIAN_SEEDS[args.seed_start:args.seed_start+args.seed_count]
    rows=[]
    for i,seed in enumerate(selected,start=1):
        print(f"[{i}/{len(selected)}] seed {seed}",flush=True)
        for fam in FAMILIES:
            rows.extend(pair_metrics_for_family_v3214(seed,fam,args.eta))

    stats=seed_stats_v3214(rows,args.nperm)
    report=summary_v3214(rows,stats)
    print(report,end="")
    args.output.write_text(report,encoding="utf-8")
    write_rows_v3214(args.csv,rows)
    write_stats_v3214(args.stats_csv,stats)
