...
def summarize_local(rows:list[dict]) -> str:
    lines=[
        '=== Soft Spaces Phase 3.2 v32.3 LOCAL HOTSPOT MECHANISM ===',
        f'Pairs/family={len(rows)//len(FAMILIES)}; total rows={len(rows)}',
        'Statistical unit: one exact +/-E pair per perturbation family.',
        'Prominence is frozen from Phase 3.1; no new hotspot score is defined.',
        '',
    ]
    candidates=[
        'real_pvq2','real_sigma','real_gap','real_pvq2_over_gap','real_sigma_over_gap',
        'delta_pvq2','delta_sigma','delta_gap','delta_pvq2_over_gap','delta_sigma_over_gap',
        'local_real_pvq2','local_real_sigma','local_real_gap','local_delta_pvq2','local_delta_sigma','local_delta_gap'
    ]
    all_ranked=[]
    for fam in FAMILIES:
        fr=[r for r in rows if r['family']==fam]
        y=np.asarray([r['prominence'] for r in fr])
        lines.append(f'FAMILY: {fam}')
        ranked=[]
        for key in candidates:
            x=np.asarray([r[key] for r in fr])
            p,s=corr_pair(x,y); ranked.append((abs(s) if np.isfinite(s) else -1,key,p,s))
        ranked.sort(reverse=True)
        for _,key,p,s in ranked:
            lines.append(f'  {key:24s} Pearson={p:+.4f}  Spearman={s:+.4f}')
            all_ranked.append((fam,key,p,s))
        # top/mid/bottom by prominence
        fr2=sorted(fr,key=lambda r:r['prominence'],reverse=True)
        n=len(fr2); cut1=(n+2)//3; cut2=(2*n+2)//3
        lines.append('  Tertile medians:')
        for name,sub in [('TOP',fr2[:cut1]),('MIDDLE',fr2[cut1:cut2]),('BOTTOM',fr2[cut2:])]:
            lines.append(f"    {name:6s} prominence={np.median([r['prominence'] for r in sub]):+.4f}; real_sigma={np.median([r['real_sigma'] for r in sub]):.4e}; real_pvq2={np.median([r['real_pvq2'] for r in sub]):.4e}; real_gap={np.median([r['real_gap'] for r in sub]):.4e}; delta_sigma={np.median([r['delta_sigma'] for r in sub]):+.4e}")
        lines.append('')
    lines += [
        'INTERPRETATION RULE:',
        '  Look for the same physically interpretable candidate ranking highly in both families.',
        '  A one-seed correlation is diagnostic evidence only, not a universal hotspot law.',
        '  If no scalar is stable across both families, the next model should combine descriptors.',
    ]
    return '\n'.join(lines)+'\n'
...
def main() -> None:
    ...
    parser.add_argument('--seed',type=int,default=HAMILTONIAN_SEEDS[0])
    parser.add_argument('--eta',type=float,default=ENERGY_REG)
    parser.add_argument('--output',type=Path,default=Path('v32_3_hotspot_local_mechanism_output.txt'))
    parser.add_argument('--csv',type=Path,default=Path('v32_3_hotspot_local_mechanism.csv'))
    args=parser.parse_args()
    if args.seed not in HAMILTONIAN_SEEDS:
        raise ValueError(f'--seed must be one of {HAMILTONIAN_SEEDS[0]}..{HAMILTONIAN_SEEDS[-1]}')
    if args.eta <= 0: raise ValueError('--eta must be > 0')
    rows=[]
    for family in FAMILIES:
        rows.extend(pair_metrics_for_family(args.seed,family,args.eta))
    report=summarize_local(rows)
    print(report,end='')
