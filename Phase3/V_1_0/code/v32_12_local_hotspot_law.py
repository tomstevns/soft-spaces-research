...
def main() -> None:
    ...
    parser.add_argument("--nrand",type=int,default=N_RANDOMIZATIONS_V3212)
    parser.add_argument("--nperm",type=int,default=N_PERM_V3212)
    parser.add_argument("--seed-start",type=int,default=0)
    parser.add_argument("--seed-count",type=int,default=len(HAMILTONIAN_SEEDS))
    parser.add_argument("--output",type=Path,default=Path("v32_12_local_hotspot_law_output.txt"))
    parser.add_argument("--csv",type=Path,default=Path("v32_12_local_hotspot_law.csv"))
    parser.add_argument("--stats-csv",type=Path,default=Path("v32_12_local_hotspot_law_stats.csv"))
    ...
    for i,seed in enumerate(selected,start=1):
        ...
        for fam in FAMILIES:
            rows.extend(pair_metrics_for_family_v3212(seed,fam,args.eta,args.nrand))

    stats=seed_law_stats(rows,args.nperm)
    report=summary_v3212(rows,stats,args.nrand,args.nperm)
    print(report,end="")
    args.output.write_text(report,encoding="utf-8")
    write_rows_v3212(args.csv,rows)
    write_stats_v3212(args.stats_csv,stats)
