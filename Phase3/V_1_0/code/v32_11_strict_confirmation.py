...
def main() -> None:
    ...
    parser.add_argument("--nrand",type=int,default=N_RANDOMIZATIONS_V3211)
    parser.add_argument("--nperm",type=int,default=N_SEED_PERMUTATIONS)
    parser.add_argument("--seed-start",type=int,default=0)
    parser.add_argument("--seed-count",type=int,default=len(HAMILTONIAN_SEEDS))
    parser.add_argument("--output",type=Path,default=Path("v32_11_strict_confirmation_output.txt"))
    parser.add_argument("--csv",type=Path,default=Path("v32_11_strict_confirmation.csv"))
    parser.add_argument("--stats-csv",type=Path,default=Path("v32_11_strict_confirmation_stats.csv"))
    ...
    for i,seed in enumerate(selected,start=1):
        ...
        for fam in FAMILIES:
            rows.extend(pair_metrics_for_family_v3211(seed,fam,args.eta,args.nrand))

    stats=strict_seed_stats(rows,args.nperm)
    report=summary_v3211(rows,stats,args.nrand,args.nperm)
    print(report,end="")
    args.output.write_text(report,encoding="utf-8")
    write_rows_v3211(args.csv,rows)
    write_strict_stats(args.stats_csv,stats)
