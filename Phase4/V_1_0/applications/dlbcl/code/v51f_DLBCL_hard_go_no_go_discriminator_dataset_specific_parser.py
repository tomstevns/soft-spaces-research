...
"""
Soft Spaces / DLBCL
v51f MASTER GO/NO-GO DISCRIMINATOR — DATASET-SPECIFIC PARSER
...
def parse_abc_gcb_labels(sample_text, gse):
    """
    v51f PARSER-ONLY FIX.

    Dataset-specific, explicit subtype extraction.

    GSE87371:
        coo: ABC -> ABC
        coo: GC  -> GCB class
        coo: GCB -> GCB class
        PMBL / Other -> unresolved

    GSE159472:
        sample title beginning "ABC" -> ABC
        sample title beginning "GCB" -> GCB
    ...
    for gsm, lines in sample_text.items():
        ...
        if gse == "GSE87371":
            ...
            evidence_lines.extend(title_lines)

            candidates = []

            for raw in title_lines:
                rhs = raw.split("=", 1)[1].strip() if "=" in raw else raw.strip()
                title = rhs.strip().strip('"').strip("'")

                if re.match(r"^ABC(?:\s|$)", title, flags=re.IGNORECASE):
                    candidates.append((1, raw))
                elif re.match(r"^GCB(?:\s|$)", title, flags=re.IGNORECASE):
                    candidates.append((0, raw))
            ...
            raise RuntimeError(
                f"No frozen subtype parser defined for {gse}."
            )
        ...
        audit_rows.append({
            ...
            "evidence_lines": " || ".join(evidence_lines),
        })
...
def main():
    ...
    print("=" * 72)
    print("Soft Spaces / DLBCL v51f GO/NO-GO DISCRIMINATOR")
...
v51f.0 — HARD GO/NO-GO DISCRIMINATOR PREREGISTRATION
...
--------------------
v51b-v51e stopped or partially stopped because cohort metadata encode
cell-of-origin differently.

v51f changes ONLY subtype metadata extraction and removes an obsolete
scikit-learn penalty argument that generated FutureWarning messages.
...
The frozen 12-gene panel, cohorts, CV splits/repeats, classifier family,
structured NULL construction, NULL count and PASS thresholds are unchanged.
...
"""
    ...
    protocol_path = docs / "v51f_0_DLBCL_GO_NO_GO_PREREGISTRATION.txt"
    protocol_path.write_text(protocol, encoding="utf-8")
    protocol_sha = sha256_file(protocol_path)

    write_json(
        docs / "v51f_0_DLBCL_GO_NO_GO_PREREGISTRATION_manifest.json",
        {
            "version": "v51f.0",
    ...
    for cohort_i, (gse, spec) in enumerate(COHORTS.items()):
        ...
        label_audit_path = (
            results
            / f"v51f_{gse}_ABC_GCB_label_parser_audit.tsv"
        )
    ...
    closure_path = docs / "v51f_1_DLBCL_GO_NO_GO_CLOSURE.txt"

    lines = [
        "Soft Spaces / DLBCL",
        "v51f.1 — HARD GO/NO-GO DISCRIMINATOR CLOSURE",
        "",
        "STATUS",
        "------",
        "CLOSED",
    ...
    manifest_path = docs / "v51f_1_DLBCL_GO_NO_GO_CLOSURE_manifest.json"
    ...
    summary_path = results / "v51f_DLBCL_go_no_go_summary.txt"

    summary = f"""=== Soft Spaces / DLBCL v51f GO/NO-GO SUMMARY ===
...
"""
    ...
    print()
    print("=" * 72)
    print("v51f DLBCL GO/NO-GO COMPLETE")
    print("=" * 72)
    print(summary)
    print("Closure:")
    print(" ", closure_path)
