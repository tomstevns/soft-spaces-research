#!/usr/bin/env python3
"""
Soft Spaces Phase 4 v41.1 — GSE10846 preprocessing for DLBCL benchmark.
...
VERSION = "v41.1-dlbcl-preprocess-1"
ACCESSION = "GSE10846"
PLATFORM = "GPL570"
...
def read_gpl_annotation(path: Path):
    """
    Read GPL570.annot.gz.

    NCBI GEO platform annotation files usually contain comment lines beginning
    with '#', followed by a tabular header.
    """
    # pandas handles gzip automatically if compression='gzip'
    df = pd.read_csv(
        path,
        sep="\t",
        compression="gzip",
    ...
    mapping = pd.DataFrame(rows, columns=["probe_id", "gene_symbol"]).drop_duplicates()
...
def main() -> int:
    ...
    print("=== Soft Spaces Phase 4 v41.1 — DLBCL preprocessing ===")
