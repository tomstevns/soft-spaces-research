...
"""
Soft Spaces / DLBCL
v51c MASTER GO/NO-GO DISCRIMINATOR — STRICT LABEL PARSER
...
def parse_abc_gcb_labels(sample_text):
    """
    STRICT PARSER-ONLY FIX for v51c.

    v51b aborted technically because it searched the complete sample metadata
    blob for the tokens ABC/GCB. That can classify a sample from a field name
    or unrelated metadata text.

    v51c uses ONLY Sample_characteristics / description-like VALUE fields and
    accepts ABC/GCB only when they appear as a phenotype VALUE associated with
    a subtype/classification/cell-of-origin concept, or when the complete
    characteristic value itself is exactly ABC or GCB.
    ...
    for gsm, lines in sample_text.items():
        ...
        for raw in lines:
            # Only metadata lines, never expression rows.
            if not (
                raw.startswith("!Sample_characteristics")
                or raw.startswith("!Sample_description")
                or raw.startswith("!Sample_title")
            ):
                continue

            # Everything right of the first '=' is the actual sample value field.
            rhs = raw.split("=", 1)[1].strip() if "=" in raw else raw.strip()
            rhs_u = rhs.upper()

            # Split "field: value" conservatively.
            if ":" in rhs:
            ...
            # Normalize surrounding punctuation but not internal content.
            value_clean = re.sub(r"^[\s\"']+|[\s\"']+$", "", value_u)

            label = None

            # Exact standalone characteristic value.
            if value_clean == "ABC":
                label = 1
            elif value_clean == "GCB":
                label = 0

            # Key/value field with relevant biological key.
            elif any(term in key_l for term in key_terms):
                abc = bool(re.search(r"(^|[^A-Z0-9])ABC([^A-Z0-9]|$)", value_u))
                gcb = bool(re.search(r"(^|[^A-Z0-9])GCB([^A-Z0-9]|$)", value_u))

                if abc and not gcb:
                    label = 1
                elif gcb and not abc:
                    label = 0

            if label is not None:
                hits.append((label, raw))

        hit_labels = sorted(set(h[0] for h in hits))
        ...
        audit_rows.append({
            "gsm": gsm,
            "parsed_label": status,
            "n_matching_lines": len(hits),
            "matching_lines": " || ".join(h[1] for h in hits),
        })
...
def main():
    ...
    print("=" * 72)
    print("Soft Spaces / DLBCL v51c GO/NO-GO DISCRIMINATOR")
...
v51c.0 — HARD GO/NO-GO DISCRIMINATOR PREREGISTRATION
...
--------------------
v51b stopped before scientific scoring because its broad metadata parser
classified GSE87371 as ABC=83, GCB=0.

v51c changes ONLY ABC/GCB metadata extraction.
...
"""
    ...
    protocol_path = docs / "v51c_0_DLBCL_GO_NO_GO_PREREGISTRATION.txt"
    protocol_path.write_text(protocol, encoding="utf-8")
    protocol_sha = sha256_file(protocol_path)

    write_json(
        docs / "v51c_0_DLBCL_GO_NO_GO_PREREGISTRATION_manifest.json",
        {
            "version": "v51c.0",
    ...
    for cohort_i, (gse, spec) in enumerate(COHORTS.items()):
        ...
        label_audit_path = (
            results
            / f"v51c_{gse}_ABC_GCB_label_parser_audit.tsv"
        )
    ...
    closure_path = docs / "v51c_1_DLBCL_GO_NO_GO_CLOSURE.txt"

    lines = [
        "Soft Spaces / DLBCL",
        "v51c.1 — HARD GO/NO-GO DISCRIMINATOR CLOSURE",
        "",
        "STATUS",
        "------",
        "CLOSED",
    ...
    manifest_path = docs / "v51c_1_DLBCL_GO_NO_GO_CLOSURE_manifest.json"
    ...
    summary_path = results / "v51c_DLBCL_go_no_go_summary.txt"

    summary = f"""=== Soft Spaces / DLBCL v51c GO/NO-GO SUMMARY ===
...
"""
    ...
    print()
    print("=" * 72)
    print("v51c DLBCL GO/NO-GO COMPLETE")
    print("=" * 72)
    print(summary)
    print("Closure:")
    print(" ", closure_path)
