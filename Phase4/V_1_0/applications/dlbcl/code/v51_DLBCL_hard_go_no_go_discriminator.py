...
"""
Soft Spaces / DLBCL
v51 MASTER GO/NO-GO DISCRIMINATOR
...
def parse_abc_gcb_labels(sample_text):
    ...
    only accept samples containing clear ABC or GCB token.
    If both appear, mark unresolved.
    """
    ...
    for gsm, lines in sample_text.items():
        ...
        abc = bool(re.search(r"\bABC\b", text))
        gcb = bool(re.search(r"\bGCB\b", text))
...
def main():
    ...
    print("=" * 72)
    print("Soft Spaces / DLBCL v51 GO/NO-GO DISCRIMINATOR")
    ...
    frozen_genes, frozen_sources, ranked_files = discover_frozen_panel(
        dlbcl_root
    )
...
v51.0 — HARD GO/NO-GO DISCRIMINATOR PREREGISTRATION
...
"""
    ...
    protocol_path = docs / "v51_0_DLBCL_GO_NO_GO_PREREGISTRATION.txt"
    protocol_path.write_text(protocol, encoding="utf-8")
    protocol_sha = sha256_file(protocol_path)

    write_json(
        docs / "v51_0_DLBCL_GO_NO_GO_PREREGISTRATION_manifest.json",
        {
            "version": "v51.0",
    ...
    for cohort_i, (gse, spec) in enumerate(COHORTS.items()):
        ...
        if len(sample_ids) < 30:
            raise RuntimeError(
                f"{gse}: too few evaluable ABC/GCB samples."
            )

        if min(n_abc, n_gcb) < CV_SPLITS:
            raise RuntimeError(
                f"{gse}: too few samples in one class for {CV_SPLITS}-fold CV."
            )
    ...
    closure_path = docs / "v51_1_DLBCL_GO_NO_GO_CLOSURE.txt"

    lines = [
        "Soft Spaces / DLBCL",
        "v51.1 — HARD GO/NO-GO DISCRIMINATOR CLOSURE",
        "",
        "STATUS",
        "------",
        "CLOSED",
    ...
    manifest_path = docs / "v51_1_DLBCL_GO_NO_GO_CLOSURE_manifest.json"
    ...
    summary_path = results / "v51_DLBCL_go_no_go_summary.txt"

    summary = f"""=== Soft Spaces / DLBCL v51 GO/NO-GO SUMMARY ===
...
"""
    ...
    print()
    print("=" * 72)
    print("v51 DLBCL GO/NO-GO COMPLETE")
    print("=" * 72)
    print(summary)
    print("Closure:")
    print(" ", closure_path)
