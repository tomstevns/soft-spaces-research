...
def run_model_repetitions(
    ...
    for rep in range(reps):
        ...
        for candidate_index, circuit in enumerate(circuits):
            p1 = run_candidate(
                simulator,
                circuit,
                shots=shots,
                seed_simulator=stable_sim_seed(
                    seed,
                    model,
                    total_cx,
                    rep,
...
def render(
    ...
    lines = [
        "=== Soft Spaces Phase 4 v40.3 QISKIT/AER CIRCUIT-LEVEL TEST ===",
    ...
    lines.extend([
        ...
        "",
        f"FINAL v40.3 PRIMARY CHECKPOINT DECISION: {'PASS' if primary['pass'] else 'FAIL'}",
        "",
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("v40_3_qiskit_aer_circuit_output.txt"),
    )
