...
* frozen reference shots: 4096
* v40.6b executed shots: 2304 (budget-adapted hardware confirmation)
* K: 8
...
Budget adaptation
-----------------
This v40.6b run deliberately uses 2304 shots/circuit instead of the frozen
4096-shot primary value, solely to remain inside the available QPU budget.
It must therefore be reported as a budget-adapted hardware confirmation,
not as the untouched 4096-shot frozen primary confirmation.
...
-----
Preflight only:
    python -X utf8 -u .\v40_6b_ibm_heron_real_null.py

Submit the frozen scientific test:
    python -X utf8 -u .\v40_6b_ibm_heron_real_null.py --submit
"""
...
VERSION = "v40.6b-heron-real-null-budget-2304-1"
...
DEFAULT_OUTPUT = Path("v40_6b_ibm_heron_real_null_result.json")
DEFAULT_PREFLIGHT = Path("v40_6b_ibm_heron_real_null_preflight.json")
...
def load_manifest(path: Path) -> dict:
    ...
    if int(protocol.get("shots", -1)) != 4096:
        die("Frozen manifest reference shots are not 4096.")
...
def json_safe_preflight(rows, summary, manifest, backend_name):
    ...
    return {
        ...
        "budget_variant": {
            "name": "v40.6b 2304-shot hardware-budget variant",
            "frozen_reference_shots": int(
                manifest["hardware_protocol"]["shots"]
            ),
            "executed_shots": BUDGET_SHOTS,
            "deviation": (
                "Shots reduced from frozen 4096 to 2304 solely to fit the "
                "Open-plan QPU budget. Seeds, REAL/NULL models, K=8, CX=20, "
                "backend, frozen physical layout, raw/no-mitigation comparison "
                "and ranking/statistical endpoints are otherwise unchanged."
            ),
            "scientific_status": (
                "Budget-adapted hardware confirmation; not the untouched "
                "4096-shot frozen primary confirmation."
            ),
        },
        "hardware_adaptation": {
            "rule": (
...
def main():
    ...
    print("=== Soft Spaces Phase 4 v40.6b IBM HERON REAL-vs-NULL (2304-shot budget variant) ===")
    ...
    print(
        "Shot protocol: frozen reference 4096 -> "
        f"budget hardware run {BUDGET_SHOTS}"
    )
    ...
    print(
        f"Shots per circuit: {summary['shots']} "
        "(budget variant; frozen reference = 4096)"
    )
