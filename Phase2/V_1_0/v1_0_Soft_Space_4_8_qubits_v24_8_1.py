...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_4_8_qubits_v24_8_1.py
...
def open_system_summary_for_batch(
    *,
    ...
    kernel_rows = build_kernel_zoom_report(contrast_pairs, kernel_top_centers, kernel_radius, kernel_min_neighbors) if (bool(contrast_focus) and bool(kernel_zoom)) else []
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v0_7 convergence harness v24.8.1 (static REAL-vs-NULL + evidence-bearing kernel-zoom dynamic comparison).")
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Gap/Isolation + Open-system (v24.8.1) ===")
    ...
    header.append("Pauli ops: lazy on-demand cache (excluding all-I); 8-qubit-safe v24.8.1 evidence merge")
    header.append("")
    header_text = "\n".join(header)
            ...
            if bool(args.open_system):
                ...
                if isinstance(osb, dict) and meta:
                    ...
                    out(
                        ...
                        f"time_mode={meta.get('time_mode')} | late_power={meta.get('time_late_power')} | contrast_focus={meta.get('contrast_focus')} | kernel_zoom={meta.get('kernel_zoom')}"
                    )
