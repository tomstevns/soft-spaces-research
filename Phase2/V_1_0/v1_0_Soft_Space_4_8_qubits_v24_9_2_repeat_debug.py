...
import argparse, importlib.util, sys, gc, traceback, os, time
...
def load_base_module():
    ...
    if not base_path.exists():
        raise FileNotFoundError(f'Base file not found next to debug wrapper: {base_path}')
    spec = importlib.util.spec_from_file_location('softspace_v24_9_2_base_debug', str(base_path))
...
def _mem_hint() -> str:
    try:
        ...
        return f"rss_mb={rss:.1f}"
    except Exception:
        return "rss_mb=unknown"

def main() -> None:
    base = load_base_module()
    ap = argparse.ArgumentParser(description='v24.9.2 repeat debug wrapper.')
    ...
    ap.add_argument('--os_pairs_per_batch', type=int, default=600)
    ap.add_argument('--os_noise_model', type=str, default='both')
    ap.add_argument('--os_gamma_phi', type=float, default=0.01)
    ap.add_argument('--os_gamma_1', type=float, default=0.02)
    ...
    ap.add_argument('--logical_basis', type=str, default='noise_diag')
    ap.add_argument('--os_noise_qubits', type=str, default='subset')
    ap.add_argument('--os_noise_subset', type=int, nargs='+', default=[3])
    ...
    ap.add_argument('--os_time_mode', type=str, default='late')
    ...
    ap.add_argument('--kernel_top_centers', type=int, default=12)
    ...
    ap.add_argument('--output', type=str, default='v24_9_2_repeat_debug_output.txt')
    args = ap.parse_args()
    ...
    with open(args.output, 'w', encoding='utf-8') as f:
        def out(s: str = ''):
            ...
            f.write(s + '\n')
            f.flush()
        out('=== v24.9.2 repeat debug wrapper ===')
        out(f'Qubits: {args.n_qubits} | repeats={args.repeat_runs} | batches/repeat={args.batches}')
        out(f'repeat_seed_stride={args.repeat_seed_stride} | batch_stride={args.batch_stride}')
        out(f'process_mem_start: {_mem_hint()}')
        out('')
        for r in range(int(args.repeat_runs)):
            rep_base_seed = int(args.base_seed + r * args.repeat_seed_stride)
            out(f'--- Repeat {r+1}/{args.repeat_runs} | base_seed={rep_base_seed} | {_mem_hint()} ---')
            t0 = time.time()
            try:
                ...
                if rf:
                    ...
                    for row in rf[:5]:
                        out(f"    {row.get('class_key','?')} | batch_count={int(row.get('batch_count',0))} | fav_rate={float(row.get('favorable_rate', float('nan'))):.2f} | stab={float(row.get('stability_score', float('nan'))):+.3f}")
                elif mr:
                ...
                out(f"Repeat {r+1} done in {time.time()-t0:.1f}s | {_mem_hint()}")
            except MemoryError as e:
                out(f"MEMORY_ERROR in repeat {r+1} | base_seed={rep_base_seed} | {_mem_hint()}")
                out(str(e))
                out(traceback.format_exc())
                break
            except Exception:
                out(f"EXCEPTION in repeat {r+1} | base_seed={rep_base_seed} | {_mem_hint()}")
                out(traceback.format_exc())
                break
            finally:
                try:
                    del real, null, scoreboards, perturb_summaries, open_system_summaries, interference_summaries
                except Exception:
                    pass
                gc.collect()
                out(f'post_gc_after_repeat_{r+1}: {_mem_hint()}')
                out('')
        out('=== End of v24.9.2 repeat debug ===')
