...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_4_8_qubits_v24_4.py
...
def open_system_summary_for_batch(
    *,
    ...
    def eval_group(model: str, which: str, pool: List[Candidate]) -> Dict[str, object]:
        ...
        leak_red = summarize_reduced_timeseries_across_pairs(t_ref, leak_curves, mode=time_mode, late_power=time_late_power, q=q_report)
        fu_red = summarize_reduced_timeseries_across_pairs(t_ref, fu_curves, mode=time_mode, late_power=time_late_power, q=q_report)
        fc_red = summarize_reduced_timeseries_across_pairs(t_ref, fc_curves, mode=time_mode, late_power=time_late_power, q=q_report)
        ...
        return {
            ...
            "reduced": {
                "mode": str(time_mode),
                "late_power": float(time_late_power),
                "leak": leak_red,
                "fid_uncond": fu_red,
                "fid_cond": fc_red,
            },
    ...
    out["_meta"] = {
        ...
        "time_late_power": float(time_late_power),
    }
    return out
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v0_7 convergence harness v24.4 (static REAL-vs-NULL + deeper dynamic/open-system REAL-vs-NULL comparison).")
    ...
    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v24_4_output.txt")
    args = ap.parse_args()
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Gap/Isolation + Open-system (v24.4) ===")
