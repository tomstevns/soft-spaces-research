...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_v25_29_upward_dimensional_validation_4q_9q.py
...
def write_physics_hotspot_predictor_report(output_path, predictions, meta):
    ...
    lines.append("v25.26 is blind 10Q prediction/discovery only; candidates must be frozen before v25.29 unseen-seed confirmation.")
    lines.append("")
...
# ----------------------------
# v25.29 unseen-seed confirmation of blind 10Q hotspot predictions
# ----------------------------
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.29|{family.upper()}|PERT|{seed}|rep{rep}"
                    )
                    dH = build_family_perturbation(
                        cache, perturb_terms_eff, seed_pert, family
                    )
...
def write_q11_dimensional_rule_test_report(output_path, summaries, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.29 PREDECLARED 11Q DIMENSIONAL-RULE TEST ===")
    ...
    lines.append("")
    lines.append("=== End of v25.29 10Q hotspot confirmation ===")
...
# ----------------------------
# v25.29 one-shot upward dimensional validation 4Q -> 9Q
# ----------------------------
...
def upward_dimensional_validation_scan(
    *,
    ...
    if q_min < 4 or q_max > 9 or q_min > q_max:
        raise ValueError("v25.29 is frozen to the range 4Q..9Q.")
    if local_step <= 0:
        raise ValueError("local_step must be > 0")
    ...
    for nq in range(q_min, q_max + 1):
        cache = PauliCache.build(nq)
        d = cache.d
        perturb_terms_eff = int(perturb_terms) if perturb_terms is not None else int(n_terms)
        frozen = [("A", chain_a[nq]), ("B", chain_b[nq])]
        frozen_set = {k for _, k in frozen}

        control_map = {}
        all_coords = []
        for branch, k in frozen:
            lo = max(0, k - local_radius)
            hi = min(d - 2, k + local_radius)
            ctrls = [c for c in range(lo, hi + 1, local_step)
                     if c != k and c not in frozen_set]
            if not ctrls:
                raise ValueError(f"No controls for {nq}Q branch {branch} k={k}")
        ...
        raw = {}
        q_seed_base = int(base_seed) + (nq - q_min) * 10_000_000

        for b in range(int(batches)):
            seed0 = q_seed_base + b * int(batch_stride)
            ...
            for s in range(int(seeds_per_batch)):
                ...
                evals, evecs_real = np.linalg.eigh(H_real)
                _, evecs_null = null_haar_basis_eigs(
                    evals, seed=seed, tag=f"NULL_HAAR_BASIS_V25_29_{nq}Q"
                )

                for family in families:
                    for rep in range(int(reps)):
                        seed_pert = stable_hash_int(
                            f"V25.29|{nq}Q|{family.upper()}|PERT|{seed}|rep{rep}"
                        )
                        dH = build_family_perturbation(
                            cache, perturb_terms_eff, seed_pert, family
                        )
                        ...
                        for k in all_coords:
                            cr = _cancel_matrix_from_basis_transform(
                                evals=evals, B=B_real, i=int(k),
                                eps_neighbor=eps_neighbor, energy_reg=energy_reg
                            )
                            cn = _cancel_matrix_from_basis_transform(
                                evals=evals, B=B_null, i=int(k),
                                eps_neighbor=eps_neighbor, energy_reg=energy_reg
                            )
                            if cr is not None and cn is not None:
                                dc = float(cr) - float(cn)
                                if np.isfinite(dc):
                                    raw.setdefault((family, int(k), b), []).append(dc)
        ...
        for branch, k in frozen:
            ...
            for b in range(int(batches)):
                ...
                for c in control_map[(branch, k)]:
                    ...
                    ct = collapsed.get(("transverse", c, b), float("nan"))
                    if np.isfinite(cd) and np.isfinite(ct):
                        neigh.append(min(float(cd), float(ct)))

                bg = _finite_median(neigh)
                if not np.isfinite(bg):
                    continue
                ...
                bgs.append(bg)
                prom.append(robust - bg)

            rate = _positive_rate(prom)
            status = "UPWARD_VALIDATED" if np.isfinite(rate) and rate >= threshold else "UPWARD_NOT_VALIDATED"
            ...
            rows.append(UpwardValidationRow(
                ...
                source_k=k,
                units=len(prom),
                prom_pos=sum(x > 0 for x in prom),
                prom_rate=rate,
    ...
    meta = {
        ...
        "chain_a": [chain_a[q] for q in range(q_min, q_max+1)],
        "chain_b": [chain_b[q] for q in range(q_min, q_max+1)],
        "rule": "k_next = 2*k + 1",
        "seeds_per_batch": seeds_per_batch, "batches": batches,
        "reps": reps, "local_radius": local_radius,
        "local_step": local_step, "threshold": threshold,
    }
    return rows, meta
...
def write_upward_dimensional_validation_report(output_path, rows, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.29 UPWARD DIMENSIONAL VALIDATION 4Q -> 9Q ===")
    lines.append(f"Frozen rule: {meta['rule']}")
    lines.append(f"Branch A: {meta['chain_a']}")
    lines.append(f"Branch B: {meta['chain_b']}")
    lines.append(
        f"Per Q: {meta['batches']} x {meta['seeds_per_batch']} seeds | reps={meta['reps']} | "
        f"local radius=+/-{meta['local_radius']} | step={meta['local_step']}"
    )
    lines.append("")
    lines.append("Frozen physics:")
    ...
    lines.append(f"Gate per Q x branch: prominence positive-rate >= {meta['threshold']:.2f}.")
    lines.append("")
    lines.append("Q | branch | k | status | units | prom pos | prom+ | med robust | med bg | med prom | min | max | dephase+ | transverse+")

    for r in rows:
        lines.append(
            f"{r.n_qubits:1d} | {r.branch:^6s} | {r.source_k:3d} | {r.status:20s} | "
            f"{r.units:5d} | {r.prom_pos:8d} | {r.prom_rate:6.3f} | "
            f"{r.med_robust:+10.6f} | {r.med_bg:+8.6f} | {r.med_prom:+9.6f} | "
            f"{r.min_prom:+8.6f} | {r.max_prom:+8.6f} | "
            f"{r.deph_rate:8.3f} | {r.trans_rate:11.3f}"
        )

    passed = sum(r.status == "UPWARD_VALIDATED" for r in rows)
    lines.append("")
    lines.append(f"Decision summary: {passed}/{len(rows)} predeclared branch-points validated across 4Q-9Q.")
    lines.append("")
    lines.append("=== End of v25.29 upward dimensional validation ===")
...
def main() -> None:
    ...
    # v25.29: unseen-seed confirmation of the exact v25.26 blind candidate list.
    ap.add_argument("--q11_dimensional_rule_test", action="store_true",
                    help="Run v25.29 unseen-seed confirmation of frozen blind 10Q hotspots and exit.")
