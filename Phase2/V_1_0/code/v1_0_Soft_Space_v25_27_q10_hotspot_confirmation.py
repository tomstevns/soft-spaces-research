...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_v25_27_q10_hotspot_confirmation.py
...
def write_physics_hotspot_predictor_report(output_path, predictions, meta):
    ...
    lines.append("v25.26 is blind 10Q prediction/discovery only; candidates must be frozen before v25.27 unseen-seed confirmation.")
    lines.append("")
...
# ----------------------------
# v25.27 unseen-seed confirmation of blind 10Q hotspot predictions
# ----------------------------
...
def q10_hotspot_confirmation_scan(
    *,
    n_qubits: int,
    from_qubits: int,
    ...
    if not frozen_candidates:
        raise ValueError("--confirm_q10_hotspots must contain at least one coordinate")
    if int(batches) < 3:
        raise ValueError("Use at least 3 batches; 8 is recommended.")
    if int(local_step) <= 0:
        raise ValueError("--q10_hotspot_local_step must be > 0")
    if not (0.0 <= float(hotspot_threshold) <= 1.0):
        raise ValueError("--q10_hotspot_threshold must be in [0,1]")
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.27|{family.upper()}|PERT|{seed}|rep{rep}"
                    )
                    dH = build_family_perturbation(
                        cache, perturb_terms_eff, seed_pert, family
                    )
    ...
    summaries.sort(
        key=lambda s: (
            1 if s.status == "Q10_HOTSPOT_CONFIRMED" else 0,
...
def write_q10_hotspot_confirmation_report(output_path, summaries, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.27 UNSEEN-SEED 10Q HOTSPOT CONFIRMATION ===")
    lines.append(f"Frozen candidates from blind v25.26 discovery: {meta['frozen_candidates']}")
    lines.append(
        f"Geometry: {meta['from_qubits']}Q source -> {meta['n_qubits']}Q target | branches={meta['branches']}"
    )
    lines.append(
    ...
    lines.append("No candidate substitution, re-ranking, score change, or post-hoc rescue.")
    lines.append(
    ...
    n_conf = sum(s.status == "Q10_HOTSPOT_CONFIRMED" for s in summaries)
    lines.append("")
    ...
    lines.append("")
    lines.append("=== End of v25.27 10Q hotspot confirmation ===")
...
def main() -> None:
    ...
    # v25.27: unseen-seed confirmation of the exact v25.26 blind candidate list.
    ap.add_argument("--q10_hotspot_confirmation", action="store_true",
                    help="Run v25.27 unseen-seed confirmation of frozen blind 10Q hotspots and exit.")
    ap.add_argument("--confirm_q10_hotspots", type=str, default="383,127,159,279,415,71")
    ap.add_argument("--q10_hotspot_local_radius", type=int, default=10)
    ap.add_argument("--q10_hotspot_local_step", type=int, default=2)
    ap.add_argument("--q10_hotspot_threshold", type=float, default=0.75)

    args = ap.parse_args()

    if bool(args.q10_hotspot_confirmation):
        if args.anchor_from_qubits is None:
            raise ValueError("--q10_hotspot_confirmation requires --anchor_from_qubits")
        candidates = _parse_candidate_list(args.confirm_q10_hotspots)
        summaries, meta = q10_hotspot_confirmation_scan(
            n_qubits=args.n_qubits,
            from_qubits=args.anchor_from_qubits,
            frozen_candidates=candidates,
            local_radius=args.q10_hotspot_local_radius,
            local_step=args.q10_hotspot_local_step,
            ...
            hotspot_threshold=args.q10_hotspot_threshold,
        )
        write_q10_hotspot_confirmation_report(args.output, summaries, meta)
        return
