...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_v25_29_1_corrected_upward_validation_4q_9q.py
...
def write_physics_hotspot_predictor_report(output_path, predictions, meta):
    ...
    lines.append("v25.26 is blind 10Q prediction/discovery only; candidates must be frozen before v25.29.1 unseen-seed confirmation.")
    lines.append("")
...
# ----------------------------
# v25.29.1 unseen-seed confirmation of blind 10Q hotspot predictions
# ----------------------------
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.29.1|{family.upper()}|PERT|{seed}|rep{rep}"
                    )
                    dH = build_family_perturbation(
                        cache, perturb_terms_eff, seed_pert, family
                    )
...
def write_q11_dimensional_rule_test_report(output_path, summaries, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.29.1 PREDECLARED 11Q DIMENSIONAL-RULE TEST ===")
    ...
    lines.append("")
    lines.append("=== End of v25.29.1 10Q hotspot confirmation ===")
...
# ----------------------------
# v25.29.1 one-shot upward dimensional validation 4Q -> 9Q
# ----------------------------
...
def _validate_source_to_target_dimension(
    *,
    ...
    if target_qubits != source_qubits + 1:
        raise ValueError("Corrected v25.29.1 requires target_qubits = source_qubits + 1")
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.29.1|{source_qubits}Q->{target_qubits}Q|"
...
def upward_dimensional_validation_scan(
    *,
    ...
    """
    if int(q_min) < 4 or int(q_max) > 9 or int(q_min) > int(q_max):
        raise ValueError("v25.29.1 target range must be 4Q..9Q")
...
def write_upward_dimensional_validation_report(output_path, rows, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.29.1 CORRECTED UPWARD DIMENSIONAL VALIDATION 4Q -> 9Q ===")
    ...
    lines.append("")
    lines.append("=== End of v25.29.1 corrected upward dimensional validation ===")
...
def main() -> None:
    ...
    # v25.29.1: unseen-seed confirmation of the exact v25.26 blind candidate list.
    ap.add_argument("--q11_dimensional_rule_test", action="store_true",
                    help="Run v25.29.1 unseen-seed confirmation of frozen blind 10Q hotspots and exit.")
    ...
    ap.add_argument("--upward_q_min", type=int, default=4)
    ap.add_argument("--upward_q_max", type=int, default=9)
    ap.add_argument("--upward_local_radius", type=int, default=4)
