...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_v25_2_q9_q12_targeted.py
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Soft Spaces Phase 2 v25.2 targeted continuation (v25.1 physics + targeted eigenpair-zone analysis).")
    ...
    ap.add_argument("--target_zones", type=str, default=None,
                    help="Phase 2: inclusive left-eigenpair index intervals, e.g. 0-511 or 0-127,384-511. Default: full spectrum.")
    ap.add_argument("--control_intervals", type=int, default=0,
                    help="Reserved Phase-2 control-interval count. v25.2 targeted baseline currently supports 0 only.")
    ...
    # Keep the first targeted implementation deliberately minimal and auditable.
    # The historical first-q9 command used --control_intervals 0.  Non-zero
    # control intervals are not silently invented here; they will be added as
    # a separate, documented extension when their exact protocol is fixed.
    if int(args.control_intervals) != 0:
        raise ValueError(
            "v25.2 targeted baseline currently supports --control_intervals 0 only. "
            "Use 0 for the first targeted runs."
        )

    header = []
    header.append("=== Soft Spaces Phase 2 v25.2 TARGETED: v25.1 baseline physics + eigenpair-zone targeting ===")
    ...
    header.append(f"Phase 2 target zones (left pair index i): {format_intervals(target_intervals)} | control_intervals={args.control_intervals}")
    ...
    header.append("Pauli ops: lazy on-demand cache (excluding all-I); v25.2 targeted continuation, baseline physics preserved")
    header.append("")
    header_text = "\n".join(header)
