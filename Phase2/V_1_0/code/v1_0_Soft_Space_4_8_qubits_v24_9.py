...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_4_8_qubits_v24_9.py
...
def build_hotspot_class_summary(hotspot_dynamic_rows: List[Dict[str, object]]) -> List[Dict[str, object]]:
    """Aggregate hotspots by position tag and coarse signature pairing to probe reproducibility as a class."""
    ...
    for row in hotspot_dynamic_rows:
        ...
        ns = tuple(row.get("null_sig_qg_coarse", []))
        key = f"pos={pos}|R={rs}|N={ns}"
    ...
    for key, rows in groups.items():
        ...
        fav = int(sum(1 for r in rows if bool(r.get("favorable", False))))
        out.append({
            "class_key": key,
            "count": int(len(rows)),
            "favorable_count": fav,
            "favorable_rate": float(fav / max(1, len(rows))),
            "dynamic_gain_med": float(np.nanmedian(dg)),
            "logical_purity_med": float(np.nanmedian(lp)),
            "structure_drift_med": float(np.nanmedian(sd)),
        })
    out.sort(key=lambda r: (-float(r["favorable_rate"]), -float(r["dynamic_gain_med"]), -int(r["count"]), r["class_key"]))
    return out
...
def open_system_summary_for_batch(
    *,
    ...
    hotspot_class_rows: List[Dict[str, object]] = []
    if hotspot_rows:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v0_7 convergence harness v24.9 (static REAL-vs-NULL + multi-point kernel-zoom dynamic comparison).")
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Gap/Isolation + Open-system (v24.9) ===")
    ...
    header.append("Pauli ops: lazy on-demand cache (excluding all-I); 8-qubit-safe v24.9 sparse hotspot evidence merge")
    header.append("")
    header_text = "\n".join(header)
        ...
        for b in range(args.batches):
            ...
            if bool(args.open_system):
                ...
                if isinstance(osb, dict) and meta:
                    ...
                    if hcls:
                        out("  Hotspot class summary (reproducibility as a class):")
                        for row in hcls[: min(6, len(hcls))]:
                            out(
                                f"    {row.get('class_key','?')} | count={int(row.get('count',0))} | favorable={int(row.get('favorable_count',0))}/{int(row.get('count',0))} | "
                                f"fav_rate={float(row.get('favorable_rate', float('nan'))):.2f} | dyn_gain_med={float(row.get('dynamic_gain_med', float('nan'))):+.4f} | "
