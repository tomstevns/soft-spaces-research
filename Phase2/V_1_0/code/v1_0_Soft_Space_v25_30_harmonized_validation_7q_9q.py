...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_v25_30_harmonized_validation_7q_9q.py
...
# ----------------------------
# v25.22 perturbation-family transfer
# ----------------------------

def build_family_perturbation(cache: PauliCache, n_terms: int, seed: int, family: str) -> np.ndarray:
    rng = rng_from_seed(seed)
    nq = int(cache.n_qubits)
    family = str(family).strip().lower()
    p = "Z" if family == "dephasing" else ("X" if family == "transverse" else None)
    if p is None:
        raise ValueError("family must be 'dephasing' or 'transverse'")
    labels=[]
    for q in range(nq):
        lab=["I"]*nq; lab[q]=p; labels.append("".join(lab))
    for q in range(nq-1):
    ...
    for k,idx in enumerate(idxs):
        op=cache.get_op(labels[int(idx)])
        H += float(coeffs[k])*op
        if cache.n_qubits>=10: del op
    return H

@dataclass(frozen=True)
class PerturbationFamilySummary:
    family: str
    n_batch_units: int
    positive_units: int
    positive_rate: float
...
def perturbation_family_transfer_scan(*,n_qubits:int,from_qubits:int,source_coordinates:List[int],
    n_terms:int,seeds_per_batch:int,batches:int,base_seed:int,batch_stride:int,
    eps_neighbor:float,reps:int,perturb_terms:Optional[int],energy_reg:float,
    cancellation_threshold:float,families:List[str]):
    cache=PauliCache.build(int(n_qubits))
    d=int(cache.d); base_dim=1<<int(from_qubits); n_branches=1<<(int(n_qubits)-int(from_qubits))
    perturb_terms_eff=int(perturb_terms) if perturb_terms is not None else int(n_terms)
    fams=[f.strip().lower() for f in families]
    all_units={}; summaries=[]
    fields=["x_path_pr","x_path_entropy","x_path_top1","x_path_top5","sigma_cancel_trace",
            "sigma_cancel_matrix","sigma_signed_trace","sigma_abs_trace_mass"]
    for family in fams:
        if family not in ("dephasing","transverse"): raise ValueError("supported: dephasing,transverse")
        raw={}
        for b in range(int(batches)):
            seed0=int(base_seed)+b*int(batch_stride)
            for s in range(int(seeds_per_batch)):
                ...
                evals,evecs_real=np.linalg.eigh(H_real)
                if cache.n_qubits>=10: cache.clear_ops()
                _,evecs_null=null_haar_basis_eigs(evals,seed=seed,tag=f"NULL_HAAR_BASIS_{family.upper()}")
                for rep in range(int(reps)):
                    seed_pert=stable_hash_int(f"V25.22|{family}|{seed}|{rep}")
                    dH=build_family_perturbation(cache,perturb_terms_eff,seed_pert,family)
                    for source_k in source_coordinates:
                        for br in range(n_branches):
                            i=int(source_k)+br*base_dim
                            if i<0 or i+1>=d: continue
                            for model,V0 in (("REAL",evecs_real),("NULL",evecs_null)):
                                desc=_path_organization_descriptors(evals=evals,evecs=V0,i=i,dH=dH,
                                    eps_neighbor=eps_neighbor,energy_reg=energy_reg)
                                if desc is not None: raw.setdefault((int(source_k),b,model),[]).append(desc)
                    del dH
                del H_real,evecs_real,evecs_null
        ...
        for b in range(int(batches)):
            ...
            if np.isfinite(rb) and np.isfinite(nb):
                rvals.append(rb); nvals.append(nb); diffs.append(rb-nb)
        rate=_positive_rate(diffs); pos=int(sum(x>0 for x in diffs))
        status="FAMILY_TRANSFER_CONFIRMED" if np.isfinite(rate) and rate>=float(cancellation_threshold) else "FAMILY_TRANSFER_NOT_CONFIRMED"
        summaries.append(PerturbationFamilySummary(family,len(diffs),pos,rate,_finite_median(rvals),_finite_median(nvals),
            _finite_median(diffs),float(np.min(diffs)) if diffs else float("nan"),float(np.max(diffs)) if diffs else float("nan"),status))
        all_units[family]=units
    meta={"version":"v25.22","families":fams,"n_qubits":int(n_qubits),"from_qubits":int(from_qubits),
          "branches":int(n_branches),"transfer_sources":[int(x) for x in source_coordinates],
          "n_terms":int(n_terms),"perturb_terms":int(perturb_terms_eff),"seeds_per_batch":int(seeds_per_batch),
          "batches":int(batches),"total_seeds":int(seeds_per_batch)*int(batches),"base_seed":int(base_seed),
          "batch_stride":int(batch_stride),"reps":int(reps),"eps_neighbor":float(eps_neighbor),
          "energy_reg":float(energy_reg),"cancellation_threshold":float(cancellation_threshold)}
    return all_units,summaries,meta

def write_perturbation_family_transfer_report(output_path,all_units,summaries,meta):
    lines=["=== Soft Spaces Phase 2 v25.22 PERTURBATION-FAMILY TRANSFER ===",
           f"Frozen source coordinates: {meta['transfer_sources']}",
           f"Geometry: {meta['from_qubits']}Q source -> {meta['n_qubits']}Q target | branches={meta['branches']}",
           f"Families: {meta['families']}",
           f"Independent seed blocks per family: {meta['batches']} x {meta['seeds_per_batch']} = {meta['total_seeds']}",
           "", "Frozen law: C_matrix(REAL) > C_matrix(NULL).",
           f"Predeclared gate per family: positive-rate >= {meta['cancellation_threshold']:.2f}.",
           "", "family | units | positive | status | Cmat+ | med REAL | med NULL | med REAL-NULL | min | max"]
    for s in summaries:
        lines.append(f"{s.family:10s} | {s.n_batch_units:5d} | {s.positive_units:8d} | {s.status:28s} | "
                     f"{s.positive_rate:6.3f} | {s.median_real_cancel:8.6f} | {s.median_null_cancel:8.6f} | "
                     f"{s.median_real_minus_null:+13.6f} | {s.min_real_minus_null:+10.6f} | {s.max_real_minus_null:+10.6f}")
    for s in summaries:
        unitmap={(u.source_k,u.batch_id,u.model):u for u in all_units[s.family]}
        lines+=["",f"Batch recurrence: {s.family}","batch | REAL Cmat | NULL Cmat | REAL-NULL | sign"]
        for b in range(int(meta["batches"])):
            r=[]; n=[]
            for k in meta["transfer_sources"]:
                ru=unitmap.get((int(k),b,"REAL")); nu=unitmap.get((int(k),b,"NULL"))
                if ru is None or nu is None: continue
                r.append(ru.sigma_cancel_matrix); n.append(nu.sigma_cancel_matrix)
            rb=_finite_median(r); nb=_finite_median(n); d=rb-nb
            lines.append(f"{b+1:5d} | {rb:10.6f} | {nb:10.6f} | {d:+10.6f} | {'POS' if d>0 else 'NEG'}")
    n_pass=sum(s.status=="FAMILY_TRANSFER_CONFIRMED" for s in summaries)
    lines+=["",f"Cross-family decision: confirmed in {n_pass}/{len(summaries)} families.",
            "","=== End of v25.22 perturbation-family transfer ==="]
    report="\n".join(lines); print(report); Path(output_path).write_text(report+"\n",encoding="utf-8")



# ----------------------------
# v25.26 physics-based hotspot predictor
# ----------------------------
...
class HotspotPrediction:
    source_k: int
    n_batch_units: int
    dephasing_delta_c: float
    transverse_delta_c: float
...
def _cancel_matrix_from_basis_transform(*, evals, B, i, eps_neighbor, energy_reg):
    """Fast v25.18-v25.22 C_matrix evaluation after B=U^dagger V U is known."""
    d = int(evals.size)
    j = int(i) + 1
    if i < 0 or j >= d:
        return None
    if float(abs(float(evals[j]) - float(evals[i]))) >= float(eps_neighbor):
        return None

    qmask = np.ones(d, dtype=bool)
    qmask[[i, j]] = False
    Wq = np.asarray(B[qmask, :][:, [i, j]], dtype=complex)
    Eq = np.asarray(evals[qmask], dtype=float)
    Eref = 0.5 * (float(evals[i]) + float(evals[j]))
    de = Eref - Eq
    reg = max(float(energy_reg), 1e-15)
    ...
    for r in range(Wq.shape[0]):
        v = np.asarray(Wq[r, :], dtype=complex).reshape(2, 1)
        A = float(inv1[r]) * (v.conj() @ v.T)
        A = _hermitize2(A)
        sumA += A
        sum_normA += float(np.linalg.norm(A, ord="fro"))

    if sum_normA <= 0.0 or not np.isfinite(sum_normA):
        return None
    return float(np.linalg.norm(sumA, ord="fro") / sum_normA)


def physics_hotspot_predictor_scan(
    *, n_qubits, from_qubits, scan_start, scan_stop, scan_step, local_radius,
    n_terms, seeds_per_batch, batches, base_seed, batch_stride, eps_neighbor,
    reps, perturb_terms, energy_reg, top_n
):
    """
    Discovery/prediction only.

    Frozen physics:
      DeltaC_f(k)=C_REAL,f(k)-C_NULL,f(k), f={dephasing, transverse}.

    Fixed cross-family score:
      S(k)=min(median_batch DeltaC_dephasing(k),
               median_batch DeltaC_transverse(k)).

    Local hotspot prominence:
      P(k)=S(k)-median nearby S(k').

    The MIN rule is conservative: a candidate must be strong in BOTH perturbation
    families already confirmed in v25.22. No fitted weights are used.
    """
    if scan_step <= 0:
        raise ValueError("--hotspot_step must be > 0")
    if scan_stop < scan_start:
        raise ValueError("--hotspot_stop must be >= --hotspot_start")
    if top_n <= 0:
        raise ValueError("--hotspot_top_n must be > 0")
    ...
    coords = [k for k in range(int(scan_start), int(scan_stop)+1, int(scan_step))
              if 0 <= k <= max_source]
    if not coords:
        raise ValueError("No legal hotspot coordinates.")
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            if cache.n_qubits >= 10:
                cache.clear_ops()
            _, evecs_null = null_haar_basis_eigs(
                evals, seed=seed, tag="NULL_HAAR_BASIS_V25_23")

            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.26|{family.upper()}|PERT|{seed}|rep{rep}")
                    dH = build_family_perturbation(
                        cache, perturb_terms_eff, seed_pert, family)

                    # Transform once, then scan many candidate P subspaces cheaply.
                    B_real = evecs_real.conj().T @ (dH @ evecs_real)
                    B_null = evecs_null.conj().T @ (dH @ evecs_null)

                    for source_k in coords:
                        crs, cns = [], []
                        for br in range(n_branches):
                            i = int(source_k) + br * base_dim
                            cr = _cancel_matrix_from_basis_transform(
                                evals=evals, B=B_real, i=i,
                                eps_neighbor=eps_neighbor, energy_reg=energy_reg)
                            cn = _cancel_matrix_from_basis_transform(
                                evals=evals, B=B_null, i=i,
                                eps_neighbor=eps_neighbor, energy_reg=energy_reg)
                            if cr is not None and cn is not None:
                                crs.append(cr); cns.append(cn)

                        if crs and cns:
                            dc = _finite_median(crs) - _finite_median(cns)
                            if np.isfinite(dc):
                                raw.setdefault((family, int(source_k), b), []).append(float(dc))

                    del B_real, B_null, dH

            del H_real, evecs_real, evecs_null

    collapsed = {key: _finite_median(vals) for key, vals in raw.items() if vals}

    med = {f: {} for f in families}
    pos = {f: {} for f in families}
    robust = {}

    for family in families:
        for k in coords:
            vals = [collapsed[(family, k, b)]
                    for b in range(int(batches))
                    if (family, k, b) in collapsed and np.isfinite(collapsed[(family, k, b)])]
            med[family][k] = _finite_median(vals)
            pos[family][k] = _positive_rate(vals)

    for k in coords:
        a, t = med["dephasing"][k], med["transverse"][k]
        robust[k] = min(a, t) if np.isfinite(a) and np.isfinite(t) else float("nan")
    ...
    for k in coords:
        ...
        if not np.isfinite(s0):
            continue
        neigh = [kk for kk in coords if kk != k and abs(kk-k) <= int(local_radius)
                 and np.isfinite(robust.get(kk, float("nan")))]
        bg = _finite_median([robust[kk] for kk in neigh])
        prominence = s0 - bg if np.isfinite(bg) else float("nan")

        rb = []
        for b in range(int(batches)):
            a = collapsed.get(("dephasing", k, b), float("nan"))
            t = collapsed.get(("transverse", k, b), float("nan"))
            if np.isfinite(a) and np.isfinite(t):
                rb.append(min(float(a), float(t)))

        predictions.append(HotspotPrediction(
            source_k=int(k), n_batch_units=len(rb),
            dephasing_delta_c=float(med["dephasing"][k]),
            transverse_delta_c=float(med["transverse"][k]),
            robust_delta_c=float(s0),
            local_background=float(bg),
            local_prominence=float(prominence),
            dephasing_positive_rate=float(pos["dephasing"][k]),
            transverse_positive_rate=float(pos["transverse"][k]),
            robust_positive_rate=float(_positive_rate(rb)),
        ))

    predictions.sort(
        key=lambda p: (
            p.local_prominence if np.isfinite(p.local_prominence) else -1e99,
            p.robust_delta_c,
            p.robust_positive_rate if np.isfinite(p.robust_positive_rate) else -1.0),
        reverse=True)

    meta = {
        "version": "v25.26", "n_qubits": int(n_qubits), "from_qubits": int(from_qubits),
        "branches": int(n_branches), "scan_start": int(scan_start), "scan_stop": int(scan_stop),
        "scan_step": int(scan_step), "n_scanned": len(coords), "local_radius": int(local_radius),
        "top_n": int(top_n), "families": list(families),
        "seeds_per_batch": int(seeds_per_batch), "batches": int(batches),
        "total_seeds": int(seeds_per_batch)*int(batches), "base_seed": int(base_seed),
        "batch_stride": int(batch_stride), "reps": int(reps),
        "perturb_terms": int(perturb_terms_eff), "energy_reg": float(energy_reg),
    }
    return predictions, meta


def write_physics_hotspot_predictor_report(output_path, predictions, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.26 PHYSICS-BASED HOTSPOT PREDICTOR ===")
    lines.append(
        f"Landscape: {meta['from_qubits']}Q source -> {meta['n_qubits']}Q target | branches={meta['branches']}")
    lines.append(
        f"Scan k={meta['scan_start']}..{meta['scan_stop']} step={meta['scan_step']} | "
        f"scanned={meta['n_scanned']} | local radius=+/-{meta['local_radius']}")
    lines.append(
        f"Discovery blocks: {meta['batches']} x {meta['seeds_per_batch']} = {meta['total_seeds']} | "
        f"base_seed={meta['base_seed']} | stride={meta['batch_stride']}")
    lines.append(
        f"Perturb reps/seed={meta['reps']} | dH_terms={meta['perturb_terms']} | "
        f"families={meta['families']} | regularizer={meta['energy_reg']}")
    lines.append("")
    lines.append("Frozen physics:")
    lines.append("  DeltaC_f(k)=C_matrix_REAL,f(k)-C_matrix_NULL,f(k).")
    lines.append("  S(k)=min(median DeltaC_dephasing, median DeltaC_transverse).")
    lines.append("  prominence=S(k)-median nearby S.")
    lines.append("v25.26 is blind 10Q prediction/discovery only; candidates must be frozen before v25.30 unseen-seed confirmation.")
    lines.append("")
    lines.append(f"Top {min(meta['top_n'], len(predictions))} predicted hotspots:")
    lines.append(
        "rank | k | units | dC dephase | dC transverse | robust dC | local bg | prominence | "
        "dephase+ | transverse+ | robust+")

    for rank, p in enumerate(predictions[:int(meta["top_n"])], 1):
        lines.append(
            f"{rank:4d} | {p.source_k:4d} | {p.n_batch_units:5d} | "
            f"{p.dephasing_delta_c:+11.6f} | {p.transverse_delta_c:+13.6f} | "
            f"{p.robust_delta_c:+9.6f} | {p.local_background:+8.6f} | "
            f"{p.local_prominence:+10.6f} | {p.dephasing_positive_rate:8.3f} | "
            f"{p.transverse_positive_rate:11.3f} | {p.robust_positive_rate:7.3f}")

    frozen = [p.source_k for p in predictions[:int(meta["top_n"])]]
    lines.append("")
    lines.append(f"FROZEN v25.24 candidate list: {frozen}")
    lines.append("")
    lines.append("Scientific reading:")
    lines.append("  1) High robust dC means REAL>NULL pathway coherence is strong in BOTH validated perturbation families.")
    lines.append("  2) Positive prominence means this cross-family excess is locally elevated relative to neighbouring coordinates.")
    lines.append("  3) These are predictions, not confirmed Soft-Space hotspots.")
    lines.append("  4) v25.24 must test exactly this frozen list on unseen seeds with the same score.")
    lines.append("")
    lines.append("=== End of v25.26 hotspot predictor ===")
...
# ----------------------------
# v25.30 unseen-seed confirmation of blind 10Q hotspot predictions
# ----------------------------

@dataclass(frozen=True)
class Q10HotspotConfirmationSummary:
    source_k: int
    n_batch_units: int
...
def q11_dimensional_rule_test_scan(
    *,
    n_qubits: int,
    from_qubits: int,
    frozen_candidates: List[int],
    local_radius: int,
    local_step: int,
    n_terms: int,
    seeds_per_batch: int,
...
) -> Tuple[List[Q10HotspotConfirmationSummary], Dict[str, Any]]:
    """
    Confirm exactly the blind v25.26 10Q candidate list on unseen seeds.

    Frozen physics:
      DeltaC_f(k) = C_REAL,f(k) - C_NULL,f(k)
      S(k) = min(DeltaC_dephasing(k), DeltaC_transverse(k))
      prominence(k) = S(k) - median nearby S

    No candidate replacement, no re-ranking, no changed score, and no post-hoc
    descriptor rescue. Candidate x batch is the recurrence unit after
    seed/rep collapse.
    """
    if not frozen_candidates:
        raise ValueError("--rule_predicted_hotspots must contain at least one coordinate")
    if int(batches) < 3:
        raise ValueError("Use at least 3 batches; 8 is recommended.")
    if int(local_step) <= 0:
        raise ValueError("--rule_local_step must be > 0")
    ...
    frozen = [int(k) for k in frozen_candidates]
    frozen_set = set(frozen)
    control_map = {}
    all_coords = []

    for k in frozen:
        if not 0 <= k <= max_source:
            raise ValueError(f"Frozen coordinate {k} outside legal source range 0-{max_source}.")
        lo = max(0, k - int(local_radius))
        hi = min(max_source, k + int(local_radius))
        ctrls = []
        for c in range(lo, hi + 1, int(local_step)):
            if c == k or c in frozen_set:
                continue
            ctrls.append(int(c))
        if not ctrls:
            raise ValueError(f"No local controls for frozen hotspot {k}.")
        control_map[k] = ctrls
        for c in [k] + ctrls:
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            _, evecs_null = null_haar_basis_eigs(
                evals, seed=seed, tag="NULL_HAAR_BASIS_V25_27"
            )

            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.30|{family.upper()}|PERT|{seed}|rep{rep}"
                    )
                    dH = build_family_perturbation(
                        cache, perturb_terms_eff, seed_pert, family
                    )
    ...
    for k in frozen:
        ...
        for b in range(int(batches)):
            kd = collapsed.get(("dephasing", k, b), float("nan"))
            kt = collapsed.get(("transverse", k, b), float("nan"))
            if not (np.isfinite(kd) and np.isfinite(kt)):
                continue

            robust_k = min(float(kd), float(kt))
            neighbour_robust = []

            for c in control_map[k]:
                cd = collapsed.get(("dephasing", c, b), float("nan"))
                ct = collapsed.get(("transverse", c, b), float("nan"))
                if np.isfinite(cd) and np.isfinite(ct):
                    neighbour_robust.append(min(float(cd), float(ct)))

            bg = _finite_median(neighbour_robust)
            if not np.isfinite(bg):
                continue

            prom = robust_k - bg
            deph_units.append(float(kd))
            trans_units.append(float(kt))
            robust_units.append(float(robust_k))
            bg_units.append(float(bg))
            prominence_units.append(float(prom))

        prate = _positive_rate(prominence_units)
        status = (
            "DIMENSIONAL_RULE_CONFIRMED"
            if np.isfinite(prate) and prate >= float(hotspot_threshold)
            else "DIMENSIONAL_RULE_NOT_CONFIRMED"
        )

        summaries.append(Q10HotspotConfirmationSummary(
            source_k=int(k),
            n_batch_units=len(prominence_units),
            positive_prominence_units=int(sum(x > 0.0 for x in prominence_units)),
            prominence_positive_rate=prate,
            median_robust_delta_c=_finite_median(robust_units),
            median_local_background=_finite_median(bg_units),
            median_prominence=_finite_median(prominence_units),
            min_prominence=float(np.min(np.asarray(prominence_units))) if prominence_units else float("nan"),
            max_prominence=float(np.max(np.asarray(prominence_units))) if prominence_units else float("nan"),
            dephasing_positive_rate=_positive_rate(deph_units),
            transverse_positive_rate=_positive_rate(trans_units),
            status=status,
        ))

    summaries.sort(
        key=lambda s: (
            1 if s.status == "DIMENSIONAL_RULE_CONFIRMED" else 0,
            s.prominence_positive_rate if np.isfinite(s.prominence_positive_rate) else -1.0,
            s.median_prominence if np.isfinite(s.median_prominence) else -1e99,
        ),
        reverse=True,
    )
...
def write_q11_dimensional_rule_test_report(output_path, summaries, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.30 PREDECLARED 11Q DIMENSIONAL-RULE TEST ===")
    lines.append(f"Frozen 11Q predictions from rule k_next=2*k+1: {meta['frozen_candidates']}")
    lines.append(
        f"Geometry: {meta['from_qubits']}Q source -> {meta['n_qubits']}Q target | branches={meta['branches']}"
    )
    lines.append(
        f"Independent seed blocks: {meta['batches']} x {meta['seeds_per_batch']} = "
        f"{meta['total_seeds']} | base_seed={meta['base_seed']} | stride={meta['batch_stride']}"
    )
    lines.append(
        f"Perturb reps/seed={meta['reps']} | families={meta['families']} | "
        f"local radius=+/-{meta['local_radius']} | step={meta['local_step']} | "
        f"regularizer={meta['energy_reg']}"
    )
    lines.append("")
    lines.append("Frozen prediction law:")
    lines.append("  DeltaC_f(k)=C_matrix_REAL,f(k)-C_matrix_NULL,f(k).")
    lines.append("  S(k)=min(DeltaC_dephasing, DeltaC_transverse).")
    lines.append("  prominence(k)=S(k)-median nearby S.")
    lines.append("Frozen dimensional rule: k_next = 2*k + 1, mapping 127->255 and 383->767. This is NOT a landscape search. No coordinate relocation, candidate substitution, re-ranking, score change, or post-hoc rescue.")
    lines.append(
        f"Predeclared gate: prominence positive-rate >= {meta['hotspot_threshold']:.2f}."
    )
    lines.append("")
    lines.append("Confirmation summary:")
    lines.append(
        "rank | k | status | units | prom pos | prom+ | med robust dC | med local bg | "
        "med prominence | min | max | dephase+ | transverse+"
    )

    for rank, s in enumerate(summaries, 1):
        lines.append(
            f"{rank:4d} | {s.source_k:4d} | {s.status:26s} | {s.n_batch_units:5d} | "
            f"{s.positive_prominence_units:8d} | {s.prominence_positive_rate:6.3f} | "
            f"{s.median_robust_delta_c:+13.6f} | {s.median_local_background:+12.6f} | "
            f"{s.median_prominence:+14.6f} | {s.min_prominence:+10.6f} | "
            f"{s.max_prominence:+10.6f} | {s.dephasing_positive_rate:8.3f} | "
            f"{s.transverse_positive_rate:11.3f}"
        )

    n_conf = sum(s.status == "DIMENSIONAL_RULE_CONFIRMED" for s in summaries)
    lines.append("")
    lines.append(f"Decision summary: blind 10Q physics-predicted hotspots confirmed = {n_conf}/{len(summaries)}.")
    lines.append("")
    lines.append("Scientific reading:")
    lines.append("  1) Confirmation means a coordinate selected blindly by the frozen 10Q predictor remains locally prominent on unseen seeds.")
    lines.append("  2) This is the 10Q analogue of the successful v25.23 -> v25.24 prediction/confirmation chain.")
    lines.append("  3) Failure is intended falsification and the candidate must not be replaced post-hoc.")
    lines.append("  4) Multiple confirmations would support dimension-dependent hotspot positions generated by a dimension-stable perturbative mechanism.")
    lines.append("")
    lines.append("=== End of v25.30 10Q hotspot confirmation ===")
...
# ----------------------------
# v25.30 one-shot upward dimensional validation 4Q -> 9Q
# ----------------------------


@dataclass(frozen=True)
class UpwardValidationRow:
    target_qubits: int
    source_qubits: int
    branch: str
    source_k: int
...
def _validate_source_to_target_dimension(
    *,
    source_qubits: int,
    target_qubits: int,
    source_hotspots: List[Tuple[str, int]],
    n_terms: int,
    seeds_per_batch: int,
    batches: int,
    base_seed: int,
    ...
    """
    Corrected validation: each source coordinate k in (n-1)Q is lifted into
    nQ using the same two-branch geometry as v25.23-v25.28:
        i = k + branch * 2^(source_qubits), branch in {0,1}.

    The REAL-NULL DeltaC is collapsed across the two lifted target branches.
    Local controls are defined in SOURCE-coordinate space and receive the same
    two-branch lift before comparison.
    """
    if target_qubits != source_qubits + 1:
        raise ValueError("Corrected v25.30 requires target_qubits = source_qubits + 1")

    cache = PauliCache.build(int(target_qubits))
    d = int(cache.d)
    source_dim = 1 << int(source_qubits)
    n_branches = 2
    max_source = source_dim - 2
    perturb_terms_eff = int(perturb_terms) if perturb_terms is not None else int(n_terms)
    families = ("dephasing", "transverse")

    frozen = [(str(branch), int(k)) for branch, k in source_hotspots]
    frozen_set = {k for _, k in frozen}

    control_map = {}
    all_source_coords = []

    for branch, k in frozen:
        if not 0 <= k <= max_source:
            raise ValueError(
                f"{source_qubits}Q->{target_qubits}Q branch {branch}: "
                f"source hotspot {k} outside legal source range 0-{max_source}"
            )
        ...
        ctrls = []
        start = lo
        if (start - k) % int(local_step) != 0:
            start += int(local_step) - ((start - k) % int(local_step))

        for c in range(start, hi + 1, int(local_step)):
            if c == k or c in frozen_set:
                continue
            ctrls.append(int(c))
        ...
        if not ctrls:
            raise ValueError(
                f"{source_qubits}Q->{target_qubits}Q branch {branch}: "
                f"no local source-space controls for hotspot {k}"
            )

        control_map[(branch, k)] = ctrls
        for c in [k] + ctrls:
            if c not in all_source_coords:
                all_source_coords.append(c)
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            _, evecs_null = null_haar_basis_eigs(
                evals,
                seed=seed,
                tag=f"NULL_HAAR_BASIS_V25_29_1_{source_qubits}Q_TO_{target_qubits}Q",
            )

            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.30|{source_qubits}Q->{target_qubits}Q|"
                        f"{family.upper()}|PERT|{seed}|rep{rep}"
                    )
                    ...
                    for source_k in all_source_coords:
                        crs, cns = [], []

                        for br in range(n_branches):
                            i = int(source_k) + br * source_dim
    ...
    for branch, k in frozen:
        ...
        for b in range(int(batches)):
            ...
            robust = min(float(kd), float(kt))
            neighbour_robust = []

            for c in control_map[(branch, k)]:
                cd = collapsed.get(("dephasing", c, b), float("nan"))
                ct = collapsed.get(("transverse", c, b), float("nan"))
            ...
            dephs.append(float(kd))
            trans.append(float(kt))
            robusts.append(float(robust))
            bgs.append(float(bg))
            prom.append(float(robust - bg))

        rate = _positive_rate(prom)
        status = (
            "UPWARD_VALIDATED"
            if np.isfinite(rate) and rate >= float(threshold)
            else "UPWARD_NOT_VALIDATED"
        )

        rows.append(UpwardValidationRow(
            target_qubits=int(target_qubits),
            source_qubits=int(source_qubits),
            branch=str(branch),
            source_k=int(k),
            units=len(prom),
            prom_pos=int(sum(x > 0.0 for x in prom)),
            prom_rate=rate,
            med_robust=_finite_median(robusts),
            med_bg=_finite_median(bgs),
            med_prom=_finite_median(prom),
            min_prom=float(np.min(np.asarray(prom))) if prom else float("nan"),
            max_prom=float(np.max(np.asarray(prom))) if prom else float("nan"),
            deph_rate=_positive_rate(dephs),
            trans_rate=_positive_rate(trans),
            status=status,
        ))
...
def upward_dimensional_validation_scan(
    *,
    q_min: int,
    q_max: int,
    n_terms: int,
    ...
    """
    Corrected predeclared validation with true source->target geometry:
    ...
    Each source k is lifted to the two target branches exactly as in the
    validated v25.23-v25.28 machinery.
    """
    if int(q_min) < 7 or int(q_max) > 9 or int(q_min) > int(q_max):
        raise ValueError("v25.30 is predeclared to harmonize target dimensions 7Q..9Q only.")

    chain_a = {4: 1, 5: 3, 6: 7, 7: 15, 8: 31, 9: 63}
    chain_b = {4: 5, 5: 11, 6: 23, 7: 47, 8: 95, 9: 191}

    rows = []

    for target_q in range(int(q_min), int(q_max) + 1):
        source_q = target_q - 1
        q_seed_base = int(base_seed) + (target_q - int(q_min)) * 10_000_000

        rows.extend(_validate_source_to_target_dimension(
            source_qubits=source_q,
            target_qubits=target_q,
            source_hotspots=[("A", chain_a[target_q]), ("B", chain_b[target_q])],
            n_terms=n_terms,
            seeds_per_batch=seeds_per_batch,
            batches=batches,
            base_seed=q_seed_base,
            batch_stride=batch_stride,
            eps_neighbor=eps_neighbor,
            reps=reps,
            perturb_terms=perturb_terms,
    ...
    meta = {
        ...
        "q_min": int(q_min),
        "q_max": int(q_max),
        "chain_a": [chain_a[q] for q in range(int(q_min), int(q_max)+1)],
        "chain_b": [chain_b[q] for q in range(int(q_min), int(q_max)+1)],
        "rule": "k_next = 2*k + 1",
        "geometry": "(n-1)Q source -> nQ target, two lifted branches",
        "seeds_per_batch": int(seeds_per_batch),
        "batches": int(batches),
        "reps": int(reps),
        "local_radius": int(local_radius),
...
def write_upward_dimensional_validation_report(output_path, rows, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.30 HARMONIZED HIGH-Q DIMENSIONAL VALIDATION 7Q -> 9Q ===")
    lines.append(f"Frozen rule: {meta['rule']}")
    lines.append(f"Geometry: {meta['geometry']}")
    lines.append("Purpose: harmonize the stable high-Q chain to the +/-10 local-control window used by later hotspot confirmations.")
    lines.append(f"Branch A: {meta['chain_a']}")
    lines.append(f"Branch B: {meta['chain_b']}")
    lines.append(
        f"Per target Q: {meta['batches']} x {meta['seeds_per_batch']} seeds | "
        f"reps={meta['reps']} | local radius=+/-{meta['local_radius']} | "
        f"step={meta['local_step']}"
    )
    lines.append("")
    lines.append("Frozen physics:")
    lines.append("  DeltaC_f(k)=C_matrix_REAL,f(k)-C_matrix_NULL,f(k).")
    lines.append("  S(k)=min(DeltaC_dephasing, DeltaC_transverse).")
    lines.append("  prominence(k)=S(k)-median nearby S.")
    lines.append("Each source-space coordinate and each local control is lifted to two target-space branches.")
    lines.append("No landscape search, no coordinate relocation, no post-hoc rescue.")
    lines.append(f"Gate per target-Q x branch: prominence positive-rate >= {meta['threshold']:.2f}.")
    lines.append("")
    lines.append(
        "sourceQ->targetQ | branch | source k | status | units | prom pos | prom+ | "
        "med robust | med bg | med prom | min | max | dephase+ | transverse+"
    )

    for r in rows:
        lines.append(
            f"{r.source_qubits:1d}Q->{r.target_qubits:1d}Q         | {r.branch:^6s} | "
            f"{r.source_k:8d} | {r.status:20s} | {r.units:5d} | "
            f"{r.prom_pos:8d} | {r.prom_rate:6.3f} | "
            f"{r.med_robust:+10.6f} | {r.med_bg:+8.6f} | "
            f"{r.med_prom:+9.6f} | {r.min_prom:+8.6f} | "
            f"{r.max_prom:+8.6f} | {r.deph_rate:8.3f} | "
            f"{r.trans_rate:11.3f}"
        )

    passed = sum(r.status == "UPWARD_VALIDATED" for r in rows)
    lines.append("")
    lines.append(
        f"Decision summary: {passed}/{len(rows)} predeclared branch-points validated "
        f"with corrected source->target branch lifting."
    )
    lines.append("")
    lines.append("=== End of v25.30 harmonized high-Q dimensional validation ===")
...
def main() -> None:
    ...
    ap.add_argument("--perturbation_family_transfer", action="store_true",
                    help="Run v25.22 across dephasing and transverse perturbation families and exit.")
    ap.add_argument("--perturbation_families", type=str, default="dephasing,transverse",
                    help="Comma-separated families: dephasing,transverse.")

    # v25.26: physics-based hotspot prediction from the frozen cancellation mechanism.
    ap.add_argument("--physics_hotspot_predictor", action="store_true",
                    help="Run v25.26 source-coordinate hotspot prediction and exit.")
    ap.add_argument("--hotspot_start", type=int, default=1)
    ap.add_argument("--hotspot_stop", type=int, default=253)
    ap.add_argument("--hotspot_step", type=int, default=2)
    ap.add_argument("--hotspot_local_radius", type=int, default=10)
    ap.add_argument("--hotspot_top_n", type=int, default=6)
    ...
    # v25.30: unseen-seed confirmation of the exact v25.26 blind candidate list.
    ap.add_argument("--q11_dimensional_rule_test", action="store_true",
                    help="Run v25.30 unseen-seed confirmation of frozen blind 10Q hotspots and exit.")
    ap.add_argument("--rule_predicted_hotspots", type=str, default="255,767")
    ap.add_argument("--rule_local_radius", type=int, default=10)
    ap.add_argument("--rule_local_step", type=int, default=2)
    ap.add_argument("--rule_threshold", type=float, default=0.75)


    ap.add_argument("--upward_dimensional_validation", action="store_true")
    ap.add_argument("--upward_q_min", type=int, default=7)
    ap.add_argument("--upward_q_max", type=int, default=9)
    ap.add_argument("--upward_local_radius", type=int, default=10)
    ap.add_argument("--upward_local_step", type=int, default=2)
    ap.add_argument("--upward_threshold", type=float, default=0.75)

    args = ap.parse_args()

    if bool(args.upward_dimensional_validation):
        rows, meta = upward_dimensional_validation_scan(
            q_min=args.upward_q_min,
            q_max=args.upward_q_max,
            ...
            local_radius=args.upward_local_radius,
            local_step=args.upward_local_step,
            threshold=args.upward_threshold,
        )
        write_upward_dimensional_validation_report(args.output, rows, meta)
        return


    if bool(args.q11_dimensional_rule_test):
        if args.anchor_from_qubits is None:
            raise ValueError("--q11_dimensional_rule_test requires --anchor_from_qubits")
        candidates = _parse_candidate_list(args.rule_predicted_hotspots)
        summaries, meta = q11_dimensional_rule_test_scan(
            n_qubits=args.n_qubits,
            from_qubits=args.anchor_from_qubits,
            frozen_candidates=candidates,
            local_radius=args.rule_local_radius,
            local_step=args.rule_local_step,
            n_terms=args.n_terms,
            seeds_per_batch=args.seeds_per_batch,
            batches=args.batches,
            base_seed=args.base_seed,
            ...
            perturb_terms=args.diag_terms,
            energy_reg=args.effective_energy_reg,
            hotspot_threshold=args.rule_threshold,
        )
        write_q11_dimensional_rule_test_report(args.output, summaries, meta)
        return
    ...
    if bool(args.physics_hotspot_predictor):
        if args.anchor_from_qubits is None:
            raise ValueError("--physics_hotspot_predictor requires --anchor_from_qubits")
        predictions, meta = physics_hotspot_predictor_scan(
            n_qubits=args.n_qubits, from_qubits=args.anchor_from_qubits,
            scan_start=args.hotspot_start, scan_stop=args.hotspot_stop,
            scan_step=args.hotspot_step, local_radius=args.hotspot_local_radius,
            n_terms=args.n_terms, seeds_per_batch=args.seeds_per_batch, batches=args.batches,
            base_seed=args.base_seed, batch_stride=args.batch_stride,
            eps_neighbor=args.eps_neighbor, reps=args.diag_reps,
            perturb_terms=args.diag_terms, energy_reg=args.effective_energy_reg,
            top_n=args.hotspot_top_n)
        write_physics_hotspot_predictor_report(args.output, predictions, meta)
        return

    if bool(args.perturbation_family_transfer):
        if args.anchor_from_qubits is None:
            raise ValueError("--perturbation_family_transfer requires --anchor_from_qubits")
        sources=_parse_candidate_list(args.transfer_sources)
        families=[x.strip() for x in args.perturbation_families.split(",") if x.strip()]
        all_units,summaries,meta=perturbation_family_transfer_scan(
            ...
            cancellation_threshold=args.cancellation_threshold,families=families)
        write_perturbation_family_transfer_report(args.output,all_units,summaries,meta)
        return
