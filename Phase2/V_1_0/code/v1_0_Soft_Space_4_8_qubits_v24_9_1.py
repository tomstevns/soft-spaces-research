...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_4_8_qubits_v24_9_1.py
...
def _summarize_hotspot_groups(groups: Dict[str, List[Dict[str, object]]], grouping: str) -> List[Dict[str, object]]:
    ...
    out.sort(key=lambda r: (-int(r["batch_count"]), -float(r["favorable_rate"]), -float(r["dynamic_gain_med"]), -int(r["count"]), r["class_key"]))
    return out
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v0_7 convergence harness v24.9.1 (static REAL-vs-NULL + sparse hotspot recurrence comparison).")
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Gap/Isolation + Open-system (v24.9.1) ===")
    ...
    header.append("Pauli ops: lazy on-demand cache (excluding all-I); 8-qubit-safe v24.9.1 sparse hotspot recurrence merge")
    header.append("")
    header_text = "\n".join(header)
    ...
    iso_eps=args.iso_eps,
        ...
        if global_hot.get("by_pos") or global_hot.get("by_pos_realcoarse"):
            out("=== Hotspot recurrence across batches (v24.9.1) ===")
            for label, rows in [("by_pos", global_hot.get("by_pos", [])), ("by_pos_realcoarse", global_hot.get("by_pos_realcoarse", []))]:
                ...
                for row in rows[: min(8, len(rows))]:
                    out(
                        ...
                        f"favorable={int(row.get('favorable_count',0))}/{int(row.get('count',0))} | fav_rate={float(row.get('favorable_rate', float('nan'))):.2f} | "
                        f"dyn_gain_med={float(row.get('dynamic_gain_med', float('nan'))):+.4f} | logical_purity_med={float(row.get('logical_purity_med', float('nan'))):+.4f} | structure_drift_med={float(row.get('structure_drift_med', float('nan'))):+.4f}"
                    )
            out("")
