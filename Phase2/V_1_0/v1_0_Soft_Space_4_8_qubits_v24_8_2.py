...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_4_8_qubits_v24_8_2.py
...
class KernelZoomRow:
    ...
    real_dom_mean: float
    null_dom_mean: float
    real_leak_mean: float
    null_leak_mean: float
    kernel_score: float
...
def build_kernel_zoom_report(
    ...
    """
    Hierarchical zoom-into-dynamic-kernels (merge version):
    identify local neighborhoods in matched REAL/NULL pair-space where static contrast is high.

    Neighborhoods are formed around top contrast centers using same-seed nearby neighbor pairs
    (|Δi|<=radius and |Δj|<=radius). This preserves the v24_7 pipeline and adds a targeted
    evidence layer instead of replacing the backend.
    """
    if not contrast_pairs or kernel_top_centers <= 0:
        return []
    radius = max(0, int(kernel_radius))
    rows: List[KernelZoomRow] = []
    used_centers: List[Tuple[int, int, int]] = []
    for cp in contrast_pairs:
        if len(rows) >= int(kernel_top_centers):
            break
        seed, ci, cj = cp.key
        # avoid near-duplicate centers in the same seed
        duplicate = False
        for s2, i2, j2 in used_centers:
            if s2 == seed and abs(ci - i2) <= radius and abs(cj - j2) <= radius:
                duplicate = True
                break
        if duplicate:
            continue
        neigh = [p for p in contrast_pairs if p.key[0] == seed and abs(p.key[1] - ci) <= radius and abs(p.key[2] - cj) <= radius]
        if len(neigh) < int(kernel_min_neighbors):
            continue
        scores = np.array([p.contrast_score for p in neigh], dtype=float)
        real_ent = np.array([0.5 * (p.real.ent_i + p.real.ent_j) for p in neigh], dtype=float)
        null_ent = np.array([0.5 * (p.null.ent_i + p.null.ent_j) for p in neigh], dtype=float)
        real_dom = np.array([p.real.dom for p in neigh], dtype=float)
        null_dom = np.array([p.null.dom for p in neigh], dtype=float)
        real_leak = np.array([p.real.leak for p in neigh], dtype=float)
        null_leak = np.array([p.null.leak for p in neigh], dtype=float)
        kernel_score = float(np.mean(scores) + 0.5 * np.max(scores) - 0.25 * np.std(scores))
        rows.append(KernelZoomRow(
            center_key=cp.key,
            seed=seed,
            i=ci,
            j=cj,
            ...
            local_std=float(np.std(scores)),
            real_ent_mean=float(np.mean(real_ent)),
            null_ent_mean=float(np.mean(null_ent)),
            real_dom_mean=float(np.mean(real_dom)),
            null_dom_mean=float(np.mean(null_dom)),
            real_leak_mean=float(np.mean(real_leak)),
            null_leak_mean=float(np.mean(null_leak)),
            kernel_score=kernel_score,
        ))
        used_centers.append(cp.key)
    rows.sort(key=lambda r: (-r.kernel_score, -r.local_best, -r.local_mean, r.center_key))
    return rows
...
def broaden_kernel_selection(
    ...
    Start from the requested kernel settings, then relax radius and neighbor threshold
    until we collect at least target_min_pairs matched local pairs, or hit max_radius.
    Returns (rows, used_radius, used_min_neighbors).
    """
    if not contrast_pairs:
        return [], int(kernel_radius), int(kernel_min_neighbors)
    radius = max(0, int(kernel_radius))
    min_neighbors = max(1, int(kernel_min_neighbors))
    target = max(1, int(target_min_pairs))

    best_rows: List[KernelZoomRow] = []
    best_cov = 0
    best_params = (radius, min_neighbors)
    visited = set()

    while True:
        key = (radius, min_neighbors)
        if key in visited:
            break
        visited.add(key)
        rows = build_kernel_zoom_report(contrast_pairs, kernel_top_centers, radius, min_neighbors)
        cov = int(sum(r.neighborhood_size for r in rows))
        if cov > best_cov:
            best_rows = rows
            best_cov = cov
            best_params = (radius, min_neighbors)
        if cov >= target and rows:
            return rows, radius, min_neighbors
        moved = False
...
def open_system_summary_for_batch(
    *,
    ...
    if bool(contrast_focus) and bool(kernel_zoom):
        target_min_pairs = max(int(os_pairs_per_batch // 4), 12) if cache.n_qubits <= 4 else max(int(os_pairs_per_batch // 5), 16)
        kernel_rows, used_kernel_radius, used_kernel_min_neighbors = broaden_kernel_selection(
            contrast_pairs,
            kernel_top_centers=max(int(kernel_top_centers), 16 if cache.n_qubits <= 4 else int(kernel_top_centers)),
            kernel_radius=int(kernel_radius),
            kernel_min_neighbors=int(kernel_min_neighbors),
            target_min_pairs=target_min_pairs,
            max_radius=5 if cache.n_qubits <= 4 else max(5, int(kernel_radius) + 2),
        )
    ...
    kernel_dynamic_rows: List[Dict[str, object]] = []
    if kernel_rows:
        kernel_dynamic_rows = build_kernel_zoom_dynamic_report(
    ...
    def eval_group(model: str, which: str, pool: List[Candidate]) -> Dict[str, object]:
        if bool(contrast_focus) and which.upper() == "Q4" and contrast_pairs:
            ...
            if kernel_rows:
                centers = {(int(r.seed), int(r.i), int(r.j)) for r in kernel_rows}
                radius_use = int(used_kernel_radius)
                local_pairs = []
                seen = set()
                for cp in contrast_pairs:
                    s,i,j = cp.key
                    for cs,ci,cj in centers:
                        if s == cs and abs(i-ci) <= radius_use and abs(j-cj) <= radius_use:
                            if cp.key not in seen:
                                seen.add(cp.key)
                                local_pairs.append(cp)
                            break
    ...
    out["_meta"] = {
        ...
        "kernel_zoom": bool(kernel_zoom), "kernel_top_centers": int(kernel_top_centers), "kernel_radius": int(kernel_radius),
        "kernel_min_neighbors": int(kernel_min_neighbors),
        "kernel_radius_used": int(used_kernel_radius),
        "kernel_min_neighbors_used": int(used_kernel_min_neighbors),
        "kernel_rows": int(len(kernel_rows)),
    }
    if kernel_rows:
        out["_kernel_zoom_static"] = [
            {
                ...
                "neighborhood_size": int(r.neighborhood_size),
                "local_mean": float(r.local_mean),
                "local_best": float(r.local_best),
                "local_std": float(r.local_std),
                "real_ent_mean": float(r.real_ent_mean),
                "null_ent_mean": float(r.null_ent_mean),
                "real_dom_mean": float(r.real_dom_mean),
                "null_dom_mean": float(r.null_dom_mean),
                "real_leak_mean": float(r.real_leak_mean),
                "null_leak_mean": float(r.null_leak_mean),
                "kernel_score": float(r.kernel_score),
            }
            for r in kernel_rows
        ]
    if kernel_dynamic_rows:
        out["_kernel_zoom"] = kernel_dynamic_rows
    return out
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v0_7 convergence harness v24.8.2 (static REAL-vs-NULL + evidence-bearing kernel-zoom dynamic comparison).")
    ...
    ap.add_argument("--output", type=str, default="v0_7_convergence_families_v24_7_output.txt")
    args = ap.parse_args()
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Gap/Isolation + Open-system (v24.8.2) ===")
    ...
    header.append("Pauli ops: lazy on-demand cache (excluding all-I); 8-qubit-safe v24.8.2 broadened kernel evidence merge")
    header.append("")
    header_text = "\n".join(header)
        ...
        for b in range(args.batches):
            ...
            if bool(args.open_system):
                ...
                if isinstance(osb, dict) and meta:
                    ...
                    kz = osb.get("_kernel_zoom", []) if isinstance(osb, dict) else []
                    if kz:
                        out("  Kernel zoom (contrast-driven local neighborhoods; dynamic evidence):")
                        for row in kz[: min(8, len(kz))]:
                            ck = row.get("center_key", [None, None, None])
                            out(
                                f"    center(seed,i,j)=({ck[0]},{ck[1]},{ck[2]}) | nbh={int(row.get('neighborhood_size', 0))} | "
                                f"contrast={float(row.get('contrast_score', float('nan'))):.4f} | local_mean={float(row.get('local_mean', float('nan'))):.4f} | "
                                f"kernel_score={float(row.get('kernel_score', float('nan'))):.4f} | dynamic_gain={float(row.get('dynamic_gain', float('nan'))):+.4f}"
                            )
                            out(
                            ...
                            out(
                                f"      best local: logical_purity={float(row.get('delta_logical_purity_best', float('nan'))):+.4f} | "
                                f"structure_drift={float(row.get('delta_structure_drift_best', float('nan'))):+.4f}"
                            )
                    kzs = osb.get("_kernel_zoom_static", []) if isinstance(osb, dict) else []
                    if kzs and not kz:
                        out("  Kernel zoom (static neighborhoods only):")
                        for row in kzs[: min(8, len(kzs))]:
                            ck = row.get("center_key", [None, None, None])
                            out(
                                f"    center(seed,i,j)=({ck[0]},{ck[1]},{ck[2]}) | nbh={int(row.get('neighborhood_size', 0))} | "
                                f"contrast={float(row.get('contrast_score', float('nan'))):.4f} | local_mean={float(row.get('local_mean', float('nan'))):.4f} | "
                                f"local_best={float(row.get('local_best', float('nan'))):.4f} | kernel_score={float(row.get('kernel_score', float('nan'))):.4f}"
                            )
                    out("")
