...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_4_8_qubits_v24_8.py
...
def main() -> None:
    ap = argparse.ArgumentParser(description="v0_7 convergence harness v24.8 (static REAL-vs-NULL + deeper dynamic/open-system REAL-vs-NULL comparison).")
    ...
    header = []
    header.append("=== v0_7: Convergence + Baseline Calibration + Perturbation Robustness + Gap/Isolation + Open-system (v24.8) ===")
    ...
    header.append("Pauli ops: lazy on-demand cache (excluding all-I); 8-qubit-safe v24.8 merge")
    header.append("")
    header_text = "\n".join(header)
        ...
        for b in range(args.batches):
            ...
            if bool(args.open_system):
                ...
                if isinstance(osb, dict) and meta:
                    ...
                    if kz:
                        out("  Kernel zoom (contrast-driven local neighborhoods):")
                        for row in kz[: min(8, len(kz))]:
                            ...
                            out(
                                f"      ent_REAL/NULL={float(row.get('real_ent_mean', float('nan'))):.3f}/{float(row.get('null_ent_mean', float('nan'))):.3f} | "
                                f"dom_REAL/NULL={float(row.get('real_dom_mean', float('nan'))):.2f}/{float(row.get('null_dom_mean', float('nan'))):.2f} | "
                                f"leak_REAL/NULL={float(row.get('real_leak_mean', float('nan'))):.3f}/{float(row.get('null_leak_mean', float('nan'))):.3f}"
                            )
                    out("")
