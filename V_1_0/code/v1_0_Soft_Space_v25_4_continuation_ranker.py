...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_v25_4_continuation_ranker.py
...
def _candidate_feature_vector(c: Candidate, d: int) -> np.ndarray:
    """
    Static REAL feature vector used for anchor compatibility:
      mean amplitude entropy,
      dominant-support fraction,
      leakage,
      interestingness score.
    """
    return np.array([
        0.5 * (float(c.ent_i) + float(c.ent_j)),
...
def continuation_rank_scan(
    *,
    ...
    """
    Static-first implementation of the Phase-1 continuation idea.
    ...
    scales = np.array([_robust_scale(real_feats[:, j]) for j in range(real_feats.shape[1])], dtype=float)
    ...
    for source_k in range(source_left_max + 1):
        ...
        anchor_compat = float(np.mean(np.exp(-zdist)))
...
def write_continuation_report(
    ...
    *,
    ...
    lines: List[str] = []
    lines.append("=== Soft Spaces Phase 2 v25.4 CONTINUATION RANKER ===")
    lines.append(
    ...
    lines.append("")
    lines.append("=== End of v25.4 continuation ranker ===")
    report = "\n".join(lines)
    print(report)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Soft Spaces Phase 2 v25.4 continuation ranker: static-first successor search from an nQ anchor into (n+1)Q.")
