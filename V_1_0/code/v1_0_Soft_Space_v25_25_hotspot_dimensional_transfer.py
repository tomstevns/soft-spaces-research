...
# -*- coding: utf-8 -*-
"""\
v1_0_Soft_Space_v25_25_hotspot_dimensional_transfer.py
...
# ----------------------------
# v25.23 physics-based hotspot predictor
# ----------------------------
...
def physics_hotspot_predictor_scan(
    *, n_qubits, from_qubits, scan_start, scan_stop, scan_step, local_radius,
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.23|{family.upper()}|PERT|{seed}|rep{rep}")
    ...
    meta = {
        "version": "v25.23", "n_qubits": int(n_qubits), "from_qubits": int(from_qubits),
...
def write_physics_hotspot_predictor_report(output_path, predictions, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.23 PHYSICS-BASED HOTSPOT PREDICTOR ===")
    lines.append(
    ...
    lines.append("v25.23 is prediction/discovery only; candidates must be frozen before unseen-seed confirmation.")
    lines.append("")
    ...
    lines.append("")
    lines.append("=== End of v25.23 hotspot predictor ===")
...
# ----------------------------
# v25.24 unseen-seed confirmation of predicted hotspots
# ----------------------------

@dataclass(frozen=True)
class HotspotConfirmationSummary:
    source_k: int
    n_batch_units: int
    ...
    *,
...
) -> Tuple[List[HotspotConfirmationSummary], Dict[str, Any]]:
    """
    v25.24 confirms the exact frozen candidates emitted by v25.23.

    Frozen score:
        DeltaC_f(k) = C_REAL,f(k) - C_NULL,f(k)
        S(k) = min(DeltaC_dephasing(k), DeltaC_transverse(k))

    Frozen hotspot property:
        prominence(k) = S(k) - median_nearby S(neighbour) > 0

    For each independent batch, seeds/reps are collapsed first.  A candidate is
    confirmed only if its local prominence is positive in at least the
    predeclared fraction of batches.

    No candidate replacement, score change, descriptor substitution, or post-hoc
    rescue is allowed.
    """
    if not frozen_candidates:
        raise ValueError("--confirm_hotspots must contain at least one frozen coordinate")
    if int(batches) < 3:
        raise ValueError("Use at least 3 batches; 8 is recommended.")
    if int(local_step) <= 0:
        raise ValueError("--hotspot_local_step must be > 0")
    ...
    for k in frozen:
        ...
        for c in range(lo, hi + 1, int(local_step)):
            ...
            ctrls.append(int(c))
        if not ctrls:
            raise ValueError(f"No local controls available for frozen hotspot {k}.")
        control_map[k] = ctrls
        for c in [k] + ctrls:
            if c not in all_coords:
                all_coords.append(c)

    # raw[(family,k,b)] -> repeated seed/rep robust C differences
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            _, evecs_null = null_haar_basis_eigs(
                evals, seed=seed, tag="NULL_HAAR_BASIS_V25_24"
            )

            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(
                        f"V25.24|{family.upper()}|PERT|{seed}|rep{rep}"
                    )
                    dH = build_family_perturbation(
                        cache, perturb_terms_eff, seed_pert, family
                    )
    ...
    for k in frozen:
        ...
        ppos = int(sum(float(x) > 0.0 for x in prominence_units))
        drate = _positive_rate(deph_units)
        trate = _positive_rate(trans_units)

        status = (
            "HOTSPOT_CONFIRMED"
            if np.isfinite(prate) and prate >= float(hotspot_threshold)
            else "HOTSPOT_NOT_CONFIRMED"
        )

        summaries.append(HotspotConfirmationSummary(
            source_k=int(k),
            n_batch_units=len(prominence_units),
            positive_prominence_units=ppos,
            ...
            min_prominence=float(np.min(np.asarray(prominence_units, dtype=float))) if prominence_units else float("nan"),
            max_prominence=float(np.max(np.asarray(prominence_units, dtype=float))) if prominence_units else float("nan"),
            dephasing_positive_rate=drate,
            transverse_positive_rate=trate,
            status=status,
        ))

    summaries.sort(
        key=lambda s: (
            1 if s.status == "HOTSPOT_CONFIRMED" else 0,
...
def write_hotspot_confirmation_report(
    output_path: str,
    summaries: List[HotspotConfirmationSummary],
    meta: Dict[str, Any],
) -> None:
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.24 UNSEEN-SEED HOTSPOT CONFIRMATION ===")
    lines.append(f"Frozen candidates from v25.23: {meta['frozen_candidates']}")
    lines.append(
        f"Geometry: {meta['from_qubits']}Q source -> {meta['n_qubits']}Q target | "
        f"branches={meta['branches']}"
    )
    lines.append(
        f"Independent seed blocks: {meta['batches']} x {meta['seeds_per_batch']} = "
    ...
    lines.append("")
    lines.append("Frozen prediction rule:")
    lines.append("  DeltaC_f(k)=C_matrix_REAL,f(k)-C_matrix_NULL,f(k).")
    lines.append("  S(k)=min(DeltaC_dephasing, DeltaC_transverse).")
    lines.append("  prominence(k)=S(k)-median nearby S.")
    lines.append("A hotspot is confirmed only by recurrent POSITIVE prominence on unseen batches.")
    lines.append("No candidate substitution, score changes, new descriptors, or post-hoc rescue.")
    lines.append(
        f"Predeclared gate: prominence positive-rate >= {meta['hotspot_threshold']:.2f}."
    )
    lines.append("")
    lines.append("Candidate confirmation summary:")
    lines.append(
        "rank | k | status | units | prom pos | prom+ | med robust dC | med local bg | "
        "med prominence | min prominence | max prominence | dephase+ | transverse+"
    )

    for rank, s in enumerate(summaries, start=1):
        lines.append(
            f"{rank:4d} | {s.source_k:4d} | {s.status:22s} | {s.n_batch_units:5d} | "
            ...
            f"{s.median_prominence:+14.6f} | {s.min_prominence:+14.6f} | "
            f"{s.max_prominence:+14.6f} | {s.dephasing_positive_rate:8.3f} | "
            f"{s.transverse_positive_rate:11.3f}"
        )

    n_conf = sum(s.status == "HOTSPOT_CONFIRMED" for s in summaries)

    lines.append("")
    lines.append("Decision summary:")
    lines.append(
        f"  Physics-predicted hotspots confirmed on unseen seeds: {n_conf}/{len(summaries)}."
    )
    lines.append("")
    lines.append("Scientific reading:")
    lines.append("  1) HOTSPOT_CONFIRMED means the precomputed coordinate remains locally elevated under the SAME frozen cross-family pathway score.")
    lines.append("  2) This is stronger than merely showing REAL>NULL globally, because confirmation requires local prominence over neighbouring coordinates.")
    lines.append("  3) HOTSPOT_NOT_CONFIRMED is an intended falsification result and the coordinate must not be replaced post-hoc.")
    lines.append("  4) Multiple confirmations support physics-first hotspot prediction: mechanism -> prediction -> unseen-seed validation.")
    lines.append("  5) If confirmed, the next step is cross-dimensional prediction/transfer using the same frozen hotspot construction.")
    lines.append("")
    lines.append("=== End of v25.24 hotspot confirmation ===")
...
# ----------------------------
# v25.25 cross-dimensional transfer of confirmed hotspots
# ----------------------------

@dataclass(frozen=True)
class HotspotTransferSummary:
    source_k: int
    n_batch_units: int
    positive_prominence_units: int
    prominence_positive_rate: float
...
def hotspot_dimensional_transfer_scan(
    *, n_qubits, from_qubits, frozen_candidates, local_radius, local_step,
    n_terms, seeds_per_batch, batches, base_seed, batch_stride, eps_neighbor,
    reps, perturb_terms, energy_reg, hotspot_threshold
):
    if not frozen_candidates:
        raise ValueError("--transfer_hotspots must contain at least one frozen coordinate")
    if int(batches) < 3:
        raise ValueError("Use at least 3 batches; 8 is recommended.")
    if int(local_step) <= 0:
        raise ValueError("--hotspot_local_step must be > 0")
    ...
    for k in frozen:
        ...
        hi = min(max_source, k + int(local_radius))
        for c in range(lo, hi + 1, int(local_step)):
            if c != k and c not in frozen_set:
                ctrls.append(int(c))
        if not ctrls:
            raise ValueError(f"No local controls for hotspot {k}.")
        control_map[k] = ctrls
        for c in [k] + ctrls:
            if c not in all_coords:
                all_coords.append(c)
    ...
    for b in range(int(batches)):
        ...
        for s in range(int(seeds_per_batch)):
            ...
            _, evecs_null = null_haar_basis_eigs(
                evals, seed=seed, tag="NULL_HAAR_BASIS_V25_25")

            for family in families:
                for rep in range(int(reps)):
                    seed_pert = stable_hash_int(f"V25.25|{family.upper()}|PERT|{seed}|rep{rep}")
                    dH = build_family_perturbation(cache, perturb_terms_eff, seed_pert, family)
                    B_real = evecs_real.conj().T @ (dH @ evecs_real)
                    B_null = evecs_null.conj().T @ (dH @ evecs_null)
    ...
    collapsed = {k: _finite_median(v) for k, v in raw.items() if v}
    summaries = []

    for k in frozen:
        prom_units, robust_units, bg_units = [], [], []
        deph_units, trans_units = [], []
        ...
        for b in range(int(batches)):
            ...
            prom = robust_k - bg
            deph_units.append(float(kd)); trans_units.append(float(kt))
            robust_units.append(float(robust_k)); bg_units.append(float(bg))
            prom_units.append(float(prom))

        prate = _positive_rate(prom_units)
        status = ("HOTSPOT_TRANSFER_CONFIRMED"
                  if np.isfinite(prate) and prate >= float(hotspot_threshold)
                  else "HOTSPOT_TRANSFER_NOT_CONFIRMED")

        summaries.append(HotspotTransferSummary(
            source_k=int(k),
            n_batch_units=len(prom_units),
            positive_prominence_units=int(sum(x > 0 for x in prom_units)),
            prominence_positive_rate=prate,
            median_robust_delta_c=_finite_median(robust_units),
            median_local_background=_finite_median(bg_units),
            median_prominence=_finite_median(prom_units),
            min_prominence=float(np.min(np.asarray(prom_units))) if prom_units else float("nan"),
            max_prominence=float(np.max(np.asarray(prom_units))) if prom_units else float("nan"),
    ...
    summaries.sort(key=lambda s: (
        1 if s.status == "HOTSPOT_TRANSFER_CONFIRMED" else 0,
        s.prominence_positive_rate if np.isfinite(s.prominence_positive_rate) else -1,
        s.median_prominence if np.isfinite(s.median_prominence) else -1e99
    ), reverse=True)
...
def write_hotspot_dimensional_transfer_report(output_path, summaries, meta):
    lines = []
    lines.append("=== Soft Spaces Phase 2 v25.25 CROSS-DIMENSIONAL HOTSPOT TRANSFER ===")
    lines.append(f"Frozen v25.24-confirmed hotspots: {meta['frozen_candidates']}")
    lines.append(f"Geometry transfer: {meta['from_qubits']}Q source -> {meta['n_qubits']}Q target | branches={meta['branches']}")
    lines.append(f"Independent seed blocks: {meta['batches']} x {meta['seeds_per_batch']} = {meta['total_seeds']} | base_seed={meta['base_seed']} | stride={meta['batch_stride']}")
    lines.append(f"Perturb reps/seed={meta['reps']} | families={meta['families']} | local radius=+/-{meta['local_radius']} | step={meta['local_step']} | regularizer={meta['energy_reg']}")
    lines.append("")
    lines.append("Frozen hotspot law:")
    lines.append("  DeltaC_f(k)=C_matrix_REAL,f(k)-C_matrix_NULL,f(k).")
    lines.append("  S(k)=min(DeltaC_dephasing, DeltaC_transverse).")
    lines.append("  prominence(k)=S(k)-median nearby S.")
    lines.append("No re-ranking, candidate replacement, score change, or post-hoc rescue.")
    lines.append(f"Predeclared gate: prominence positive-rate >= {meta['hotspot_threshold']:.2f}.")
    lines.append("")
    lines.append("Transfer summary:")
    lines.append("rank | k | status | units | prom pos | prom+ | med robust dC | med local bg | med prominence | min | max | dephase+ | transverse+")

    for rank, s in enumerate(summaries, 1):
        lines.append(
            f"{rank:4d} | {s.source_k:4d} | {s.status:30s} | {s.n_batch_units:5d} | "
            f"{s.positive_prominence_units:8d} | {s.prominence_positive_rate:6.3f} | "
            f"{s.median_robust_delta_c:+13.6f} | {s.median_local_background:+12.6f} | "
    ...
    n_conf = sum(s.status == "HOTSPOT_TRANSFER_CONFIRMED" for s in summaries)
    lines.append("")
    lines.append(f"Decision summary: {n_conf}/{len(summaries)} frozen hotspots survive dimensional transfer.")
    lines.append("")
    lines.append("Scientific reading:")
    lines.append("  1) Confirmation means a 9Q predicted+confirmed hotspot remains locally prominent at 10Q under the same frozen physics.")
    lines.append("  2) This is coordinate-specific dimensional transfer, not merely the global REAL>NULL law.")
    lines.append("  3) Failure must not be rescued by moving the coordinate after inspecting 10Q.")
    lines.append("  4) Success supports physics-first hotspot predictability across dimension.")
    lines.append("")
    lines.append("=== End of v25.25 cross-dimensional hotspot transfer ===")
...
def main() -> None:
    ...
    # v25.23: physics-based hotspot prediction from the frozen cancellation mechanism.
    ap.add_argument("--physics_hotspot_predictor", action="store_true",
                    help="Run v25.23 source-coordinate hotspot prediction and exit.")
    ...
    # v25.24: unseen-seed confirmation of the exact v25.23 frozen candidate list.
    ap.add_argument("--hotspot_confirmation", action="store_true",
                    help="Run v25.24 unseen-seed confirmation of frozen physics-predicted hotspots and exit.")
    ap.add_argument("--confirm_hotspots", type=str, default="63,191,99,161,179,133",
                    help="Comma-separated frozen candidate coordinates from v25.23.")
    ap.add_argument("--hotspot_local_step", type=int, default=2)
    ap.add_argument("--hotspot_threshold", type=float, default=0.75,
                    help="Required fraction of independent batches with positive local prominence.")

    # v25.25: dimensional transfer of the two v25.24-confirmed hotspots.
    ap.add_argument("--hotspot_dimensional_transfer", action="store_true")
    ap.add_argument("--transfer_hotspots", type=str, default="63,191")
    ...
    if bool(args.hotspot_dimensional_transfer):
        if args.anchor_from_qubits is None:
            raise ValueError("--hotspot_dimensional_transfer requires --anchor_from_qubits")
        candidates = _parse_candidate_list(args.transfer_hotspots)
        summaries, meta = hotspot_dimensional_transfer_scan(
            n_qubits=args.n_qubits,
            from_qubits=args.anchor_from_qubits,
            frozen_candidates=candidates,
            local_radius=args.hotspot_local_radius,
            local_step=args.hotspot_local_step,
        ...
        write_hotspot_dimensional_transfer_report(args.output, summaries, meta)
        return

    if bool(args.hotspot_confirmation):
        if args.anchor_from_qubits is None:
            raise ValueError("--hotspot_confirmation requires --anchor_from_qubits")
        candidates = _parse_candidate_list(args.confirm_hotspots)
        summaries, meta = hotspot_confirmation_scan(
            n_qubits=args.n_qubits,
            from_qubits=args.anchor_from_qubits,
            frozen_candidates=candidates,
            local_radius=args.hotspot_local_radius,
        ...
        write_hotspot_confirmation_report(args.output, summaries, meta)
        return
